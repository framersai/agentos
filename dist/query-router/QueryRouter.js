...
export class QueryRouter {
    ...
    constructor(config) {
         ...
         * planning.
         */
        this.capabilityDiscoveryEngine = null;
     ...
     */
    async classify(query, conversationHistory, options) {
     ...
     * @throws If the router has not been initialised via {@link init}.
     */
    async route(query, conversationHistory, options) {
        this.ensureInitialized();
        ...
        const trimmedHistory = this.trimConversationHistory(conversationHistory);
        const [classification, executionPlan] = await this.classifier.classifyWithPlan(query, trimmedHistory, options);
        ...
        if (executionPlan.skills.length > 0 ||
        ...
        const recommendations = executionPlan.skills.length > 0 ||
            executionPlan.tools.length > 0 ||
            executionPlan.extensions.length > 0
            ? {
        ...
        if (result.recommendations) {
            ...
            const parts = [];
            if (r.skills.length)
                parts.push(`skills: ${r.skills.map((s) => s.skillId).join(', ')}`);
            if (r.tools.length)
                parts.push(`tools: ${r.tools.map((t) => t.toolId).join(', ')}`);
            if (r.extensions.length)
                parts.push(`extensions: ${r.extensions.map((e) => e.extensionId).join(', ')}`);
            if (parts.length)
                console.log(`[QueryRouter] recommendations | ${parts.join(' | ')}`);
        }
        this.emit({
            type: 'route:complete',
            result,
    ...
    getPlatformKnowledgeCounts() {
        ...
        for (const chunk of this.corpus) {
            if (!chunk.sourcePath.startsWith('platform:'))
                continue;
            counts.total += 1;
            if (chunk.sourcePath.startsWith('platform:tools/'))
                counts.tools += 1;
            else if (chunk.sourcePath.startsWith('platform:skills/'))
                counts.skills += 1;
            else if (chunk.sourcePath.startsWith('platform:faq/'))
                counts.faq += 1;
            else if (chunk.sourcePath.startsWith('platform:api/'))
                counts.api += 1;
            else if (chunk.sourcePath.startsWith('platform:troubleshooting/'))
