...
function matchesMode(ruleModes, mode) {
    if (!ruleModes || ruleModes.length === 0)
        return true;
...
function resolveAddonPromptContent(key, addonPrompts) {
...
function resolvePresetById(config, presetId) {
    return config.presets.find((preset) => preset.id === presetId) ?? null;
}
function pickPresetIdByRules(config, input) {
    const msgLen = input.userMessage?.length ?? 0;
    const sortedRules = [...(config.rules || [])].sort((a, b) => (b.priority || 0) - (a.priority || 0));
    for (const rule of sortedRules) {
        if (!matchesMode(rule.modes, input.mode))
            continue;
        if (typeof rule.minMessageChars === 'number' && msgLen < rule.minMessageChars)
            continue;
        if (typeof rule.maxMessageChars === 'number' && msgLen > rule.maxMessageChars)
            continue;
        if (!containsAny(input.userMessage, rule.anyKeywords))
            continue;
        if (!containsAll(input.userMessage, rule.allKeywords))
            continue;
...
function resolveSystemInstructions(config, preset) {
...
export function selectPromptProfile(config, input, previousState) {
    const now = typeof input.now === 'number' ? input.now : Date.now();
    const reviewEvery = Math.max(1, config.routing.reviewEveryNTurns || 1);
    const shouldForceReview = Boolean(input.forceReview) || (Boolean(input.didCompact) && config.routing.forceReviewOnCompaction);
    if (previousState && !shouldForceReview && previousState.turnsSinceReview < reviewEvery) {
        const cachedPreset = resolvePresetById(config, previousState.presetId) ??
    ...
    const preset = resolvePresetById(config, picked.presetId) ??
        resolvePresetById(config, config.routing.defaultPresetId) ??
        {
            id: config.routing.defaultPresetId,
            addonPromptKeys: [],
...
export const DEFAULT_PROMPT_PROFILE_CONFIG = {
    version: '1.0.0',
    routing: {
        reviewEveryNTurns: 6,
...
//# sourceMappingURL=PromptProfileRouter.js.map