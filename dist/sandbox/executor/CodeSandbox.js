...
import { SandboxError, } from './ICodeSandbox.js';
// ============================================================================
// Constants
// ============================================================================
...
export class CodeSandbox {
     ...
     */
    async initialize(logger, defaultConfig) {
        this.logger = logger;
        if (defaultConfig) {
        ...
        try {
            ...
            switch (request.language) {
                ...
                case 'shell':
                    ...
                    break;
                default:
                    throw new SandboxError(`Language "${request.language}" is not currently supported`, 'error', executionId);
            }
            // Update stats
     ...
     */
    async executeJavaScript(executionId, request, config, startedAt, startTime) {
        let stdout = '';
        let stderr = '';
     ...
     */
    async executePython(executionId, request, config, startedAt, startTime) {
        ...
        try {
            ...
            const isTimeout = typeof error === 'object' && error !== null && 'timedOut' in error && error.timedOut;
            return {
                executionId,
            ...
            catch { /* temp file may not exist if writeFileSync failed */ }
        }
    }
    /**
     ...
     */
    async executeShell(executionId, request, config, startedAt, startTime) {
