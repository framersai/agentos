...
export class ConversationVerifier {
     ...
     */
    async verifyConversation(conversationId, publicKeyBase64) {
        // Get all events related to this conversation
        const conversationEvents = await this.ledger.getEventsByPayloadFilter({ conversationId }, undefined);
        // Also check for events with table references to this conversation
        const messageEvents = await this.ledger.getEventsByPayloadFilter({ recordId: conversationId }, ['conversation.created', 'conversation.archived', 'conversation.tombstoned']);
     ...
     * @param publicKeyBase64 - Optional public key for signature verification.
     */
    async verifyMessage(messageId, publicKeyBase64) {
        // Find events referencing this message
        const events = await this.ledger.getEventsByPayloadFilter({ recordId: messageId });
        if (events.length === 0) {
            return {
                valid: false,
                eventsVerified: 0,
     ...
     */
    async getProvenanceSummary(conversationId) {
        const conversationEvents = await this.ledger.getEventsByPayloadFilter({ conversationId });
        const allEvents = await this.ledger.getAllEvents();
        ...
        return {
            ...
            revisionEvents: conversationEvents.filter(e => e.type === 'message.revised' || e.type === 'memory.revised').length,
            tombstoneEvents: conversationEvents.filter(e => e.type === 'message.tombstoned' || e.type === 'memory.tombstoned' || e.type === 'conversation.tombstoned').length,
...
//# sourceMappingURL=ConversationVerifier.js.map