...
export class AnchorManager {
    constructor(storageAdapter, ledger, keyManager, config, tablePrefix = '', provider) {
        this.timer = null;
     ...
     */
    async createAnchor(fromSequence, toSequence) {
        ...
        // Persist anchor
        await this.storageAdapter.run(`INSERT INTO ${this.tablePrefix}anchors
     ...
     */
    async publishExternally(anchor) {
        if (!this.provider)
            return;
        try {
            const result = await this.provider.publish(anchor);
            if (result.success && result.externalRef) {
                await this.storageAdapter.run(`UPDATE ${this.tablePrefix}anchors SET external_ref = ? WHERE id = ?`, [result.externalRef, anchor.id]);
     ...
     */
    async getLastAnchor() {
        const row = await this.storageAdapter.get(`SELECT * FROM ${this.tablePrefix}anchors ORDER BY sequence_to DESC LIMIT 1`);
     ...
     */
    async getAllAnchors() {
        const rows = await this.storageAdapter.all(`SELECT * FROM ${this.tablePrefix}anchors ORDER BY sequence_from ASC`);
        return rows.map(r => this.rowToAnchor(r));
    }
    /**
     * Get the anchor covering a specific sequence number.
     */
    async getAnchorForSequence(sequence) {
        const row = await this.storageAdapter.get(`SELECT * FROM ${this.tablePrefix}anchors
       WHERE sequence_from <= ? AND sequence_to >= ?
       LIMIT 1`, [sequence, sequence]);
        return row ? this.rowToAnchor(row) : null;
    }
    /**
    ...
    async verifyAnchor(anchorId) {
        const row = await this.storageAdapter.get(`SELECT * FROM ${this.tablePrefix}anchors WHERE id = ?`, [anchorId]);
        if (!row) {
        ...
        if (events.length !== anchor.eventCount) {
            errors.push(`Event count mismatch: anchor says ${anchor.eventCount}, found ${events.length}`);
        }
        ...
        if (anchor.signature) {
            try {
                ...
                const sigValid = await AgentKeyManager.verifySignature(anchor.merkleRoot, anchor.signature, pubKey);
                if (!sigValid) {
...
//# sourceMappingURL=AnchorManager.js.map