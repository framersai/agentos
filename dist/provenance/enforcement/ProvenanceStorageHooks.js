...
function isUpsertLikeMutation(statement) {
    ...
    if (/INSERT\s+OR\s+REPLACE\s+INTO\s+/i.test(statement))
        return true;
    // `INSERT ... ON CONFLICT ... DO UPDATE` (SQLite/Postgres) mutates existing rows.
    if (/ON\s+CONFLICT[\s\S]*DO\s+UPDATE/i.test(statement))
        return true;
    return false;
}
            ...
            switch (mode) {
                case 'sealed': {
                    ...
                    if (operation === 'UPDATE' || operation === 'DELETE' || isUpsert) {
                        throw new ProvenanceViolationError(`${isUpsert ? 'UPSERT' : operation} operations are forbidden in sealed mode on table '${table}'`, { code: 'SEALED_MUTATION_BLOCKED', table, operation });
                    }
                    break;
                }
                case 'revisioned':
