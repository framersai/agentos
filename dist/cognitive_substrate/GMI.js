...
import { MetapromptExecutor } from './MetapromptExecutor.js';
...
export class GMI {
    ...
    async initialize(persona, config) {
        ...
        if (this.cognitiveMemory) {
            this.memoryBridge = new CognitiveMemoryBridge(this.cognitiveMemory, () => this.currentGmiMood, () => this.currentUserContext, () => this.getCurrentPrimaryPersonaId(), () => this.gmiId, (type, message, details) => this.addTraceEntry(type, message, details));
        }
        else {
            this.memoryBridge = null;
        }
        ...
        this.sentimentTracker = new SentimentTracker(this.utilityAI, this.workingMemory, () => this.activePersona, () => this.conversationHistoryManager.history, () => this.reasoningTrace.entries, () => this.currentUserContext, async (ctx) => {
            this.currentUserContext = ctx;
            await this.workingMemory.set('currentUserContext', this.currentUserContext);
        }, (type, message, details) => this.addTraceEntry(type, message, details), () => this.gmiId);
        ...
        this.metapromptExecutor = new MetapromptExecutor({
            ...
            addTraceEntry: (type, message, details) => this.addTraceEntry(type, message, details),
            getModelAndProvider: (preferredModel, preferredProvider) => this.getModelAndProviderForLLMCall(preferredModel, preferredProvider, this.activePersona.defaultModelId || this.config.defaultLlmModelId, this.activePersona.defaultProviderId || this.config.defaultLlmProviderId),
            onMoodUpdate: (mood) => {
    ...
    shouldTriggerRAGRetrieval(query, context) {
        ...
        if (retrievalTriggers.onIntentDetected?.length && context?.detectedIntents?.length) {
            const matched = retrievalTriggers.onIntentDetected.some(intent => context.detectedIntents.includes(intent));
            if (matched)
                return true;
        }
        return false;
     ...
     */
    determinePromptFormat(modelDetails, providerId) {
     ...
     */
    determineToolFormat(modelDetails, providerId) {
        const pid = (modelDetails?.providerId || providerId || '').toLowerCase();
        if (pid.includes('anthropic'))
            return 'anthropic_tools';
        if (pid.includes('google') || pid.includes('gemini'))
            return 'google_function_calling';
    ...
    async *processTurnStream(turnInput) {
        ...
        try {
            if (turnInput.userContextOverride) {
                const mergedPreferences = turnInput.userContextOverride.preferences &&
            ...
            main_processing_loop: while (safetyBreak < maxToolLoopIterations) {
                ...
                const discoveryPromptContext = typeof turnInput.metadata?.capabilityDiscovery?.promptContext === 'string'
                ...
                const skillPromptContext = typeof turnInput.metadata?.skillPromptContext === 'string'
                ...
                const plannedToolFailureMode = turnInput.metadata?.executionPolicy?.toolFailureMode === 'fail_closed'
                    ? 'fail_closed'
                    : 'fail_open';
                const plannedToolSelectionMode = turnInput.metadata?.executionPolicy?.toolSelectionMode === 'discovered'
                    ? 'discovered'
                    : 'all';
                ...
                if (plannedToolSelectionMode === 'discovered' &&
                    capabilityDiscoveryResult &&
                    typeof this.toolOrchestrator.listDiscoveredTools === 'function') {
                    const discoveredTools = await this.toolOrchestrator.listDiscoveredTools(capabilityDiscoveryResult, {
                        personaId: this.activePersona.id,
                        personaCapabilities: this.activePersona.allowedCapabilities || [],
                        userContext: this.currentUserContext,
                    });
                ...
                if (currentIterationToolCallRequests.length > 0) {
                    ...
                    for (const toolCallReq of currentIterationToolCallRequests) {
                        ...
                        if (result.isError && plannedToolFailureMode === 'fail_closed') {
                            throw new GMIError(`Tool '${toolCallReq.name}' failed and execution policy is fail_closed.`, GMIErrorCode.TOOL_ERROR, {
