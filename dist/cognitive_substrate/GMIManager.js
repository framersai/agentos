...
import { ReasoningEntryType } from './IGMI.js';
...
export class GMIManager {
    constructor(config, subscriptionService, authService, conversationManager, promptEngine, llmProviderManager, utilityAI, toolOrchestrator, retrievalAugmentor, personaLoader) {
    ...
    validateGMIDependencies() {
        const check = (service, name, code = GMIErrorCode.DEPENDENCY_ERROR) => {
            if (!service)
                throw new GMIManagerError(`${name} dependency is missing.`, code, { service: name });
        };
    ...
    resolvePersonaWithAgencyOverlay(persona, agencyOptions) {
        ...
        if (agencyOptions.evolutionRules && agencyOptions.evolutionRules.length > 0) {
            const context = agencyOptions.evolutionContext ?? {
    ...
    async resolveTierByName(tierName) {
    ...
    async userMeetsPersonaTier(userId, persona) {
    ...
    async loadAllPersonaDefinitions() {
        if (this.isInitialized)
            console.log(`GMIManager (ID: ${this.managerId}): Refreshing persona definitions...`);
        try {
            ...
            try {
                ...
                if (this.toolOrchestrator && typeof this.toolOrchestrator.listAvailableTools === 'function') {
                    try {
                        ...
                        knownToolIds = new Set(availableTools.flatMap((tool) => {
                            const identifiers = [];
                        ...
                        console.warn(`GMIManager (ID: ${this.managerId}): Failed to enumerate registered tools for persona validation.`, toolListError);
                    }
                }
                const validationOpts = {
                    ...
                    tokenEstimator: (text) => this.promptEngine.estimateTokenCount(text)
                };
                ...
                const strictConfig = this.config.personaValidationStrict || { enabled: false };
    ...
    stripSensitivePersonaData(persona) {
        const { baseSystemPrompt: _baseSystemPrompt, defaultModelId: _defaultModelId, defaultProviderId: _defaultProviderId, defaultModelCompletionOptions: _defaultModelCompletionOptions, promptEngineConfigOverrides: _promptEngineConfigOverrides, embeddedTools: _embeddedTools, metaPrompts: _metaPrompts, initialMemoryImprints: _initialMemoryImprints, contextualPromptElements: _contextualPromptElements, ...publicPersonaData } = persona;
        const stripped = {
            ...publicPersonaData,
    ...
    deriveRequiredSecretsForPersona(persona) {
        const secretIds = new Set();
        persona.modelTargetPreferences?.forEach((preference) => {
            const secretId = resolveSecretForProvider(preference.providerId) ??
    ...
    async assembleGMIBaseConfig(persona, runtimeContext) {
        ...
        if (this.config.cognitiveMemoryFactory) {
            try {
                ...
                console.warn(`GMIManager (ID: ${this.managerId}): Failed to initialize cognitive memory for GMI ${runtimeContext.gmiInstanceId}.`, error?.message ?? error);
            }
        }
        return {
            workingMemory,
    ...
    async getOrCreateGMIForSession(userId, sessionId, requestedPersonaId, conversationIdInput, preferredModelId, preferredProviderId, userApiKeys, agencyOptions) {
        this.ensureInitialized();
        ...
        if (strictConfig?.enabled && !strictConfig.shadowMode && personaRecord?.status === 'invalid') {
            throw new GMIManagerError(`Persona '${requestedPersonaId}' is invalid and blocked by strict validation. Reasons: ${personaRecord.blockedReasons?.join(', ') || 'unknown'}`, GMIErrorCode.PERMISSION_DENIED, { requestedPersonaId, blockedReasons: personaRecord.blockedReasons });
        }
        ...
        const { persona: effectivePersona, overlay, overlayChanged } = this.resolvePersonaWithAgencyOverlay(personaDefinition, agencyOptions);
        let gmiInstanceId = this.gmiSessionMap.get(sessionId);
        let gmi = gmiInstanceId ? this.activeGMIs.get(gmiInstanceId) : undefined;
        if (gmi && (gmi.getPersona().id !== requestedPersonaId || overlayChanged)) {
            console.log(`GMIManager (ID: ${this.managerId}): Persona refresh requested for session ${sessionId} (base '${requestedPersonaId}', overlayChanged=${overlayChanged}). Recreating GMI.`);
        ...
        if (gmi && gmiInstanceId) {
            ...
            currentConversationContext = await this.conversationManager.getOrCreateConversationContext(conversationIdInput || sessionId, userId, gmiInstanceId, personaDefinition.id);
        }
        else {
            ...
            currentConversationContext = await this.conversationManager.getOrCreateConversationContext(conversationIdInput || sessionId, userId, newGmiInstanceId, personaDefinition.id);
        }
        if (!gmi) {
        ...
        if (preferredModelId)
            currentConversationContext.setMetadata('preferredModelId', preferredModelId);
        if (preferredProviderId)
            currentConversationContext.setMetadata('preferredProviderId', preferredProviderId);
        if (currentConversationContext.getMetadata('userId') !== userId)
            currentConversationContext.setMetadata('userId', userId);
        if (currentConversationContext.getMetadata('gmiInstanceId') !== gmi.gmiId)
            currentConversationContext.setMetadata('gmiInstanceId', gmi.gmiId);
        if (currentConversationContext.getMetadata('activePersonaId') !== gmi.getPersona().id)
            currentConversationContext.setMetadata('activePersonaId', gmi.getPersona().id);
    ...
    getGMIByInstanceId(gmiInstanceId) {
        this.ensureInitialized();
        return this.activeGMIs.get(gmiInstanceId);
    }
    async deactivateGMIForSession(sessionId) {
        this.ensureInitialized();
    ...
    async cleanupInactiveGMIs(inactivityThresholdMinutes) {
        this.ensureInitialized();
    ...
    async processUserFeedback(userId, sessionId, personaId, feedbackData) {
        this.ensureInitialized();
    ...
    addTraceEntryToRelevantGMI(sessionId, type, message, details) {
        const gmiInstanceId = this.gmiSessionMap.get(sessionId);
        if (gmiInstanceId) {
            const gmi = this.activeGMIs.get(gmiInstanceId);
            if (gmi && typeof gmi['addTraceEntry'] === 'function') {
                const entryDetails = details ?? {};
                gmi['addTraceEntry'](type, message, entryDetails);
            }
        }
    }
}
//# sourceMappingURL=GMIManager.js.map