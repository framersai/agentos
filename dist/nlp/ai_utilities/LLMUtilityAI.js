...
export class LLMUtilityAI {
    ...
    getModelAndProvider(taskSpecificModelId, taskSpecificProviderId) {
        ...
        if (!modelId)
            throw new GMIError("LLMUtilityAI: No modelId configured for task or as default.", GMIErrorCode.CONFIG_ERROR);
    ...
    async makeLLMCall(prompt, systemPrompt, taskHint, modelId, providerId, completionOptions) {
        ...
        const canGenerate = provider && typeof provider.generate === 'function';
        const canGenerateCompletion = provider && typeof provider.generateCompletion === 'function';
        if (!canGenerate && !canGenerateCompletion) {
            throw new GMIError(`Provider '${effectiveProviderId}' is not initialized or does not support generation.`, GMIErrorCode.PROVIDER_NOT_FOUND);
        }
        const finalOptions = {
            temperature: 0.3,
            maxTokens: 1024,
        ...
        if (canGenerateCompletion) {
            ...
            const completionResult = await provider.generateCompletion(effectiveModelId, messages, finalOptions);
    ...
    async summarize(textToSummarize, options) {
        ...
        if (options?.desiredLength === 'short')
            lengthInstruction = "a very short, concise summary (1-2 sentences)";
        else if (options?.desiredLength === 'long')
            lengthInstruction = "a detailed, longer summary (multiple paragraphs if needed)";
        else if (typeof options?.desiredLength === 'number') {
            if (options.desiredLength <= 1 && options.desiredLength > 0)
                lengthInstruction = `a summary that is approximately ${Math.round(options.desiredLength * 100)}% of the original length`;
            else if (options.desiredLength > 1)
                lengthInstruction = `a summary of about ${options.desiredLength} sentences`;
        }
        ...
        return this.makeLLMCall(textToSummarize.substring(0, options?.maxInputLength || 8000), // Limit input for safety
        systemPrompt, 'summarization', options?.modelId || this.config.summarizationModelId, options?.methodOptions?.providerId, // Allow provider override for summarization
        ...
        try {
            ...
            if (options?.attemptFixWithLLM) {
                ...
                try {
                    ...
                    const fixedJsonString = await this.makeLLMCall(fixPrompt, "You are an expert JSON correction assistant. Output only valid JSON.", 'json_fix', options.llmModelIdForFix || this.config.jsonFixerModelId, options.llmProviderIdForFix, { temperature: 0.0, maxTokens: Math.min(4000, Math.max(500, jsonString.length * 2)) } // Give enough tokens for fixed output
                    );
                    try {
...
Respond with ONLY a JSON object with "bestClass" (string or array of strings if multiLabel=${!!options.multiLabel}), "confidence" (float 0-1, or array of floats for multiLabel), and optionally "allScores" (array of {classLabel: string, score: float}).`;
        const responseText = await this.makeLLMCall(textToClassify, systemPrompt, 'classification', options.modelId || this.config.classificationModelId, options.methodOptions?.providerId, { temperature: 0.2, responseFormat: { type: "json_object" }, ...options.methodOptions });
        const parsed = await this.parseJsonSafe(responseText, { attemptFixWithLLM: true, llmModelIdForFix: this.config.jsonFixerModelId });
        ...
        return {
            ...
            allScores: parsed.allScores || options.candidateClasses.map(c => ({
                classLabel: c,
                score: (Array.isArray(parsed.bestClass) ? parsed.bestClass.includes(c) : parsed.bestClass === c)
                    ? (Array.isArray(parsed.confidence) ? parsed.confidence[parsed.bestClass.indexOf(c)] || 0.5 : parsed.confidence)
...
Respond with ONLY a JSON array of strings, e.g., ["keyword1", "keyphrase two"].`;
        const responseText = await this.makeLLMCall(textToAnalyze, systemPrompt, 'keyword_extraction', options?.modelId || this.config.keywordModelId, options?.methodOptions?.providerId, { temperature: 0.3, responseFormat: { type: "json_object" }, ...options?.methodOptions });
        const parsed = await this.parseJsonSafe(responseText, { attemptFixWithLLM: true, llmModelIdForFix: this.config.jsonFixerModelId });
...
Respond with ONLY a JSON object containing "score" (float, -1.0 to 1.0, where -1 is very negative, 1 is very positive), "polarity" ("positive", "negative", or "neutral"), and optionally "intensity" (float 0-1). Provide "positiveTokens" and "negativeTokens" as arrays of strings if identifiable.`;
        const responseText = await this.makeLLMCall(text, systemPrompt, 'sentiment_analysis', options?.modelId || this.config.sentimentModelId, options?.methodOptions?.providerId, { temperature: 0.3, responseFormat: { type: "json_object" }, ...options?.methodOptions });
        const parsed = await this.parseJsonSafe(responseText, { attemptFixWithLLM: true, llmModelIdForFix: this.config.jsonFixerModelId });
...
Respond with ONLY a JSON array of objects, each with "language" (BCP-47 code like "en-US", "es", "zh-CN") and "confidence" (float 0-1), sorted by confidence. Limit to ${maxCandidates} result(s). If unsure, return an empty array or a guess with low confidence.`;
        const responseText = await this.makeLLMCall(text.substring(0, 1000), // Send a decent sample for detection
        systemPrompt, 'language_detection', options?.modelId || this.config.languageDetectionModelId, options?.methodOptions?.providerId, { temperature: 0.1, responseFormat: { type: "json_object" }, ...options?.methodOptions });
        const parsed = await this.parseJsonSafe(responseText, { attemptFixWithLLM: true, llmModelIdForFix: this.config.jsonFixerModelId });
        if (!parsed || !Array.isArray(parsed)) {
    ...
    async tokenize(text, options) {
        ...
        if (options?.toLowerCase !== false)
            processedText = processedText.toLowerCase(); // Default true
    ...
    async stemTokens(tokens, _options) {
        this.ensureInitialized();
...
Respond with ONLY a JSON object: {"similarityScore": float_value_between_0_and_1}.`;
        ...
        const responseText = await this.makeLLMCall(combinedText, systemPrompt, 'semantic_similarity', options?.llmModelId || this.config.semanticSimilarityModelId, options?.llmProviderId, { temperature: 0.1, responseFormat: { type: "json_object" } });
        const parsed = await this.parseJsonSafe(responseText, { attemptFixWithLLM: true, llmModelIdForFix: this.config.jsonFixerModelId });
    ...
    async normalizeText(text, options) {
        ...
        if (options?.toLowerCase !== false)
            instructions += "Convert to lowercase. ";
        if (options?.expandContractions)
            instructions += "Expand common contractions (e.g., 'don't' to 'do not'). ";
        if (options?.removePunctuation)
            instructions += "Remove punctuation unless it's essential for meaning (like in numbers or acronyms). ";
        if (options?.stripHtml)
            instructions += "Remove HTML tags. ";
        ...
        return this.makeLLMCall(text, systemPrompt, 'text_normalization', options?.language === 'en' ? (this.config.textNormalizationModelId || this.config.defaultModelId) : options?.language ? undefined : this.config.defaultModelId, // Use general model or allow specific
        undefined, // Provider
        { temperature: 0.0 });
    }
    async generateNGrams(tokens, options) {
        ...
        nValues.forEach(n => {
            ...
            for (let i = 0; i < limit; i++) {
                ...
                if (!options.includePartial && ngram.length < n)
                    continue;
                if (ngram.length > 0)
                    ngrams.push(ngram);
            }
...
Text: "${text.substring(0, 1500)}..."`; // Send a substantial sample
        const responseText = await this.makeLLMCall(text, // Full text for estimation
        systemPrompt, 'readability_estimation', this.config.readabilityEstimationModelId || this.config.defaultModelId, undefined, { temperature: 0.1, responseFormat: { type: "json_object" } });
        const parsed = await this.parseJsonSafe(responseText, { attemptFixWithLLM: true, llmModelIdForFix: this.config.jsonFixerModelId });
    ...
    async summarizeConversationHistory(messages, targetTokenCount, modelInfo, preserveImportantMessages) {
        this.ensureInitialized();
        ...
        const summaryText = (await this.makeLLMCall(serializedHistory, systemPrompt, 'conversation_summary', this.config.summarizationModelId, this.config.defaultProviderId, { maxTokens: Math.max(256, targetTokenCount), temperature: 0.2 })).trim();
    ...
    async summarizeRAGContext(context, targetTokenCount, modelInfo, preserveSourceAttribution) {
        this.ensureInitialized();
        ...
        const summaryText = (await this.makeLLMCall(serialized, systemPrompt, 'rag_context_summary', this.config.summarizationModelId, this.config.defaultProviderId, { maxTokens: Math.max(256, targetTokenCount), temperature: 0.2 })).trim();
        return {
            summary: summaryText,
    ...
    async checkHealth() {
        ...
        const providerHealth = typeof this.llmProviderManager?.checkHealth === 'function'
            ...
            : undefined;
        const providerManagerHealthy = providerHealth?.isOverallHealthy ??
            this.llmProviderManager?.isInitialized ??
...
//# sourceMappingURL=LLMUtilityAI.js.map