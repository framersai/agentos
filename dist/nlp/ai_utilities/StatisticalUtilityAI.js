...
export class StatisticalUtilityAI {
    createStemmerRegistry() {
        const registry = {
            porter: natural.PorterStemmer,
            lancaster: natural.LancasterStemmer,
        };
        const naturalWithVariants = natural;
        for (const [suffix, langCode] of Object.entries(NATURAL_STEMMER_LANGUAGE_KEYS)) {
            ...
            if (porterVariant && typeof porterVariant.stem === 'function') {
                registry[`porter_${langCode}`] = porterVariant;
            }
        }
        const carryFrench = naturalWithVariants.CarryStemmerFr;
        if (carryFrench && typeof carryFrench.stem === 'function') {
            registry.carry_fr = carryFrench;
        }
        const stemmerId = naturalWithVariants.StemmerId;
        if (stemmerId && typeof stemmerId.stem === 'function') {
            registry.default_id = stemmerId;
        }
        const stemmerJa = naturalWithVariants.StemmerJa;
        if (stemmerJa && typeof stemmerJa.stem === 'function') {
            registry.default_ja = stemmerJa;
        }
        return registry;
    }
    constructor(utilityId) {
        ...
        this.stopWords = new Map([['en', DEFAULT_ENGLISH_STOP_WORDS]]);
    ...
    getSentimentAnalyzer(language) {
        const langCode = language.toLowerCase().split('-')[0];
        if (!this.sentimentAnalyzers.has(langCode)) {
            try {
                const stemmer = langCode === 'en' ? natural.PorterStemmer : undefined;
                const analyzer = new natural.SentimentAnalyzer(langCode, stemmer, 'afinn');
                ...
                console.warn(`StatisticalUtilityAI (ID: ${this.utilityId}): Failed to initialize sentiment analyzer for '${langCode}' (${e?.message}). Falling back to neutral analyzer.`);
                const neutralAnalyzer = {
                    getSentiment: (_tokens) => 0,
                    vocabulary: {},
                };
                this.sentimentAnalyzers.set(langCode, neutralAnalyzer);
            }
        }
        return this.sentimentAnalyzers.get(langCode);
    }
    // Helper to load lexicon (conceptual for now, needs actual file reading)
    loadSentimentLexiconFromFile(filePath) {
        ...
        if (method === 'first_n_sentences') {
            ...
            if (options?.desiredLength) {
                if (options.desiredLength === 'short')
                    numSentences = 2;
                else if (options.desiredLength === 'medium')
                    numSentences = Math.max(3, Math.floor(sentences.length * 0.3));
                else if (options.desiredLength === 'long')
                    numSentences = Math.max(4, Math.floor(sentences.length * 0.5));
    ...
    async classifyText(textToClassify, options) {
        ...
        if (options.method === 'naive_bayes' && classifier) {
            try {
                ...
                if (!tokens)
                    throw new GMIError("Tokenization failed for classification.", GMIErrorCode.PROCESSING_ERROR);
                ...
                let bestClasses = best.label;
                let confidences = best.value;
                if (options.multiLabel) {
                    ...
                    if (bestClasses.length === 0 && classifications.length > 0)
                        bestClasses = [classifications[0].label]; // Fallback
    ...
    async tokenize(text, options) {
        ...
        if (shouldLowerCase)
            processedText = processedText.toLowerCase(); // Default true for word tokenization
    ...
    async calculateSimilarity(text1, text2, options) {
        ...
        if (method === 'jaccard') {
            ...
            const allTerms = new Set([...termsInDoc1.keys(), ...termsInDoc2.keys()]);
            const vecA = [];
            const vecB = [];
    ...
    async analyzeSentiment(text, options) {
        ...
        if (score > 0.05)
            polarity = 'positive'; // Thresholds from 'natural' are typically small
        else if (score < -0.05)
            polarity = 'negative';
        ...
        const lexicon = analyzerEntry ? analyzerEntry.vocabulary : undefined;
        if (lexicon) {
            tokens.forEach(t => {
                ...
                if (tokenScore !== undefined) {
                    ...
                    if (tokenScore > 0)
                        positiveTokens.push({ token: t, score: tokenScore });
                    else if (tokenScore < 0)
                        negativeTokens.push({ token: t, score: tokenScore });
                }
            });
        }
        return {
    ...
    async trainModel(trainingData, modelType, // e.g., "text_classifier_naive_bayes"
    trainingOptions) {
        this.ensureInitialized();
        ...
        const classifier = new natural.BayesClassifier(stemmer, alpha);
    ...
    async saveTrainedModel(modelId, modelType, storagePath) {
        this.ensureInitialized();
    ...
    async loadTrainedModel(modelId, modelType, storagePath) {
        this.ensureInitialized();
        ...
        return new Promise((resolve) => {
            natural.BayesClassifier.load(loadPath, this.getStemmer('porter'), (err, classifier) => {
                if (err || !classifier) {
...
//# sourceMappingURL=StatisticalUtilityAI.js.map