 ...
 */
import { isLikelyCodeBlock, partitionCodeAndProse, recombineCodeAndProse, } from './interfaces.js';
import { OpenAITranslationProvider } from './providers/OpenAITranslationProvider.js';
import { DeepLTranslationProvider } from './providers/DeepLTranslationProvider.js';
import { WhisperDetectionProvider } from './providers/WhisperDetectionProvider.js';
export class LanguageService {
    constructor(config) {
        this.config = config;
        this.detectionProviders = [];
    ...
    async initialize() {
        ...
        if (this.config.autoDetect) {
            ...
            for (const d of (this.config.detectionProviderConfigs || [])) {
                if (d.id === 'whisper_stub')
                    this.detectionProviders.push(new WhisperDetectionProvider(d.id, d.params || {}));
        ...
        for (const t of (this.config.translationProviderConfigs || [])) {
            if (t.id === 'openai_chat') {
                const apiKey = process.env.OPENAI_API_KEY || t.params?.apiKey;
                if (apiKey)
                    this.translationProviders.push(new OpenAITranslationProvider(t.id, { apiKey, model: t.params?.model }));
            }
            else if (t.id === 'deepl') {
                const apiKey = process.env.DEEPL_API_KEY || t.params?.apiKey;
                if (apiKey)
                    this.translationProviders.push(new DeepLTranslationProvider(t.id, { apiKey }));
            }
        }
        // Always ensure at least one provider
        if (!this.translationProviders.length)
            this.translationProviders.push(new NoOpTranslationProvider('noop_translation'));
        ...
        for (const p of this.detectionProviders) {
            try {
                if (!p.isInitialized)
                    await p.initialize();
            }
    ...
    async detectLanguages(text) {
        if (!this.initialized || !this.config.autoDetect)
            return [];
        const aggregated = [];
    ...
    negotiate(params) {
        ...
        if (params.explicitUserLanguage)
            path.push(`explicit:${params.explicitUserLanguage}`);
        if (params.conversationPreferred)
            path.push(`conversation:${params.conversationPreferred}`);
        if (params.personaDefault)
            path.push(`persona:${params.personaDefault}`);
        ...
        if (!target) {
            if (params.preferSourceLanguageResponses && supportedSet.has(source)) {
                target = source;
                path.push('preferSource');
            }
            else if (params.personaDefault && supportedSet.has(params.personaDefault.toLowerCase())) {
                target = params.personaDefault.toLowerCase();
                path.push('personaDefault');
            }
            else if (supportedSet.has(source)) {
                target = source;
                path.push('sourceSupported');
            }
            else {
    ...
    async maybeNormalizeForPivot(content, source, pivot) {
        if (!pivot || pivot === source)
            return null;
    ...
    async maybeTranslateForDisplay(content, source, target) {
        if (source === target)
            return null;
        return this.performTranslation(content, source, target, { domain: 'general' });
    }
    async translateQueryForRag(query, source, pivot) {
        if (source === pivot)
            return null;
        return this.performTranslation(query, source, pivot, { domain: 'rag' });
    }
    async translateRagResults(results, target) {
        const out = [];
    ...
    async translateToolArguments(args, source, toolLanguage) {
        if (source === toolLanguage)
            return null;
    ...
    async translateToolResult(result, source, target) {
        if (source === target)
            return null;
    ...
    async performTranslation(input, source, target, options) {
        if (!this.initialized)
            return null;
        if (!this.translationProviders.length)
            return null;
...
class BasicHeuristicDetectionProvider {
    constructor(id, defaultLang) {
        this.id = id;
    ...
    async detect(text) {
        ...
        const results = [];
        for (const [code, rx] of checks)
            if (rx.test(lower))
                results.push({ code, confidence: 0.6 });
        if (results.length === 0)
            results.push({ code: this.defaultLang, confidence: 0.5 });
...
class NoOpTranslationProvider {
    ...
    async initialize() { this.isInitialized = true; }
    async translate(input, source, target) {
