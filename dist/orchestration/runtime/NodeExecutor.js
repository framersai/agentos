...
export class NodeExecutor {
    ...
    async execute(node, state) {
        if (node.timeout) {
            ...
            if (!result.success &&
                ...
                const onTimeout = node.executorConfig.onTimeout;
                if (onTimeout === 'accept') {
                    const timeoutAcceptResult = {
                        success: true,
                        output: { approved: true, decidedBy: 'timeout-accept' },
                    };
                    const guardrailCheck = await this.runHumanNodeGuardrails(node.id, node.executorConfig, timeoutAcceptResult.output);
     ...
     */
    async executeHuman(nodeId, config) {
     ...
     */
    async runHumanNodeGuardrails(nodeId, config, approvalOutput) {
        ...
        const evalResult = await this.deps.guardrailEngine.evaluate({ prompt: config.prompt, approval: approvalOutput }, guardrailIds);
        if (!evalResult.passed) {
            const reason = evalResult.results
                .map((r) => r.reason)
                .filter(Boolean)
     ...
     */
    async executeSubgraph(config, state) {
     ...
     */
    async executeExtension(config, state) {
        if (!this.deps.extensionExecutor) {
            return { success: true, output: 'extension-not-configured' };
        }
        try {
            const result = await this.deps.extensionExecutor(config.extensionId, config.method, { input: state.input, scratch: state.scratch });
            return {
