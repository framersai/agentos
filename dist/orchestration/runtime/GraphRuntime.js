...
function shouldTriggerPlannerReevaluation(reevalInterval, completedNodeCount) {
...
function getParallelBatchSizeBeforePlannerReevaluation(reevalInterval, completedNodeCountBeforeBatch, readyNodeCount) {
    if (typeof reevalInterval !== 'number'
        || !Number.isFinite(reevalInterval)
        || reevalInterval <= 0
        || readyNodeCount <= 0) {
    ...
    const remainingUntilBoundary = completedSinceLastBoundary === 0
        ? reevalInterval
...
export class GraphRuntime {
    ...
    async *stream(graph, input) {
        ...
        while (true) {
            ...
            const parallelBatchSize = readyNodes.length > 1
                ? (this.config.expansionHandler
                    ? getParallelBatchSizeBeforePlannerReevaluation(this.config.reevalInterval, completedNodes.length, readyNodes.length)
                    : readyNodes.length)
                : readyNodes.length;
            if (parallelBatchSize > 1) {
                ...
                const branchResults = [];
                const parallelOutcomes = await Promise.all(parallelReadyNodes.map(async (nodeId) => {
                    ...
                    if (node.checkpoint === 'before' || node.checkpoint === 'both') {
                        const checkpointId = await this.saveCheckpoint(activeGraph, runId, nodeId, branchState, nodeResults, completedNodes, skippedNodes, []);
                    ...
                    if (result.interrupt) {
                        ...
                        return { nodeId, events, branchState, earlyTermination: { type: 'interrupt', events: terminationEvents } };
                    }
                    if (!result.success) {
                        ...
                        return { nodeId, events, branchState, earlyTermination: { type: 'error', events: terminationEvents } };
                    }
                    events.push({ type: 'node_end', nodeId, output: result.output, durationMs });
                    ...
                    if (node.checkpoint === 'after' || node.checkpoint === 'both' || graph.checkpointPolicy === 'every_node') {
                        const checkpointId = await this.saveCheckpoint(activeGraph, runId, nodeId, branchState, nodeResults, completedNodes, skippedNodes, this.resolvePendingEdgeIds(outEdges, targets));
                        branchState = this.attachCheckpointMetadata(branchState, checkpointId);
                        events.push({ type: 'checkpoint_saved', checkpointId, nodeId });
                    }
                    return {
                ...
                for (const outcome of parallelOutcomes) {
                    ...
                    if (outcome.earlyTermination) {
                        ...
                        const checkpointId = await this.saveCheckpoint(activeGraph, runId, outcome.nodeId, outcome.branchState, nodeResults, completedNodes, skippedNodes, []);
                ...
                if (this.config.expansionHandler) {
                    ...
                    for (const outcome of parallelOutcomes) {
                        if (!outcome || outcome.earlyTermination || !outcome.expansionRequests?.length)
                            continue;
                    ...
                    if (expansionQueue.length > 0) {
                        ...
                        const checkpointIdBefore = await this.saveCheckpoint(activeGraph, runId, checkpointNodeId, state, nodeResults, completedNodes, skippedNodes, []);
                ...
                // ── Checkpoint BEFORE ────────────────────────────────────────────────
                if (node.checkpoint === 'before' || node.checkpoint === 'both') {
                    const checkpointId = await this.saveCheckpoint(activeGraph, runId, nodeId, state, nodeResults, completedNodes, skippedNodes, []);
                ...
                if (this.config.expansionHandler && expansionRequests.length > 0) {
                    let checkpointIdBefore;
                    for (const request of expansionRequests) {
                        if (!checkpointIdBefore) {
                            checkpointIdBefore = await this.saveCheckpoint(activeGraph, runId, nodeId, state, nodeResults, completedNodes, skippedNodes, []);
                            state = this.attachCheckpointMetadata(state, checkpointIdBefore);
                ...
                if (node.checkpoint === 'after' ||
                    ...
                    const checkpointId = await this.saveCheckpoint(activeGraph, runId, nodeId, state, nodeResults, completedNodes, skippedNodes, this.resolvePendingEdgeIds(outEdges, targets));
                    state = this.attachCheckpointMetadata(state, checkpointId);
                    yield { type: 'checkpoint_saved', checkpointId, nodeId };
                }
            }
     ...
     */
    async resume(graph, runOrCheckpointId) {
        let finalOutput;
     ...
     * @throws {Error} When no checkpoint exists for the given identifier.
     */
    async *streamResume(graph, runOrCheckpointId) {
        const checkpoint = await this.config.checkpointStore.latest(runOrCheckpointId)
            ?? await this.config.checkpointStore.get(runOrCheckpointId);
        if (!checkpoint)
            throw new Error(`No checkpoint found for identifier ${runOrCheckpointId}`);
     ...
     */
    async evaluateEdges(edges, state, result) {
            ...
            switch (edge.type) {
                ...
                case 'personality':
                    if (edge.personalityCondition) {
                        ...
                        const traitValue = this.config.personaTraits?.[trait]
                            ?? state.scratch?._personaTraits?.[trait]
                            ?? 0.5;
     ...
     */
    async saveCheckpoint(graph, runId, nodeId, state, nodeResults, visitedNodes, skippedNodes, pendingEdges) {
        const checkpoint = {
    ...
    attachCheckpointMetadata(state, checkpointId) {
        return {
            ...state,
            checkpointId,
            diagnostics: {
    ...
    resolvePendingEdgeIds(edges, targets) {
        return edges
...
function shouldRetry(policy, errorMessage) {
...
function evaluateConditionExpression(expr, state) {
    return safeEvaluateExpression(expr, state);
}
function getResultEvents(result) {
