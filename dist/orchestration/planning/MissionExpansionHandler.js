...
function normalizeEdge(raw, index) {
    return {
        ...
        ...(typeof raw.discoveryKind === 'string' ? { discoveryKind: raw.discoveryKind } : {}),
        ...(typeof raw.discoveryFallback === 'string' ? { discoveryFallback: raw.discoveryFallback } : {}),
        ...(isRecord(raw.personalityCondition)
            ? { personalityCondition: raw.personalityCondition }
            : {}),
        ...(isRecord(raw.guardrailPolicy)
            ? { guardrailPolicy: raw.guardrailPolicy }
            : {}),
...
function normalizeGraphPatch(raw) {
    const patch = raw;
    ...
    return {
        ...
        removeNodes: Array.isArray(patch.removeNodes)
            ? patch.removeNodes.filter((value) => typeof value === 'string')
            : [],
        ...
        estimatedCostDelta: typeof patch.estimatedCostDelta === 'number' && Number.isFinite(patch.estimatedCostDelta)
            ? patch.estimatedCostDelta
            : 0,
        estimatedLatencyDelta: typeof patch.estimatedLatencyDelta === 'number' && Number.isFinite(patch.estimatedLatencyDelta)
...
function describeRole(node) {
    if (node.executorConfig.type === 'gmi')
        return node.executorConfig.instructions;
...
export function createMissionExpansionHandler(options) {
    ...
    const assignProviders = (patch) => {
        ...
        if (gmiNodes.length > 0 && availableProviders.length > 0) {
            try {
                const assignments = new ProviderAssignmentEngine(availableProviders).assign(gmiNodes.map((node) => ({
                    ...node,
    ...
    const generatePatch = async (request, graph, requesterNodeId, nodeResults) => {
        if (request.patch) {
        ...
        const completedNodesJson = JSON.stringify(Object.entries(nodeResults).map(([nodeId, value]) => ({
            nodeId,
        ...
        const response = await options.llmCaller('You are evaluating a mission graph expansion. Respond with JSON only.', prompt);
        const parsed = extractJson(response);
        if (!parsed.shouldExpand || !parsed.patch)
            return null;
    ...
    return {
        handle: async (context) => {
            const patch = await generatePatch(context.request, context.graph, context.nodeId, context.nodeResults);
            if (!patch) {
                return null;
            }
            ...
            for (const rewire of assignedPatch.rewireEdges ?? []) {
                const sourceEdge = context.graph.edges.find((edge) => edge.source === rewire.from && edge.target === rewire.to);
                events.push({
...
//# sourceMappingURL=MissionExpansionHandler.js.map