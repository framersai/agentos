 ...
 */
export type ChannelPlatform = 'telegram' | 'whatsapp' | 'discord' | 'slack' | 'webchat' | 'signal' | 'imessage' | 'google-chat' | 'teams' | 'matrix' | 'zalo' | 'email' | 'sms' | 'nostr' | 'twitch' | 'line' | 'feishu' | 'mattermost' | 'nextcloud-talk' | 'tlon' | 'irc' | 'zalouser' | 'twitter' | 'instagram' | 'reddit' | 'pinterest' | 'tiktok' | 'youtube' | 'linkedin' | 'facebook' | 'threads' | 'bluesky' | 'mastodon' | 'devto' | 'hashnode' | 'medium' | 'wordpress' | 'farcaster' | 'lemmy' | 'google-business' | (string & {});
/**
 ...
 */
export type ChannelCapability = 'text' | 'rich_text' | 'images' | 'video' | 'audio' | 'voice_notes' | 'documents' | 'stickers' | 'reactions' | 'threads' | 'typing_indicator' | 'read_receipts' | 'group_chat' | 'channels' | 'buttons' | 'inline_keyboard' | 'embeds' | 'mentions' | 'editing' | 'deletion' | 'stories' | 'reels' | 'hashtags' | 'polls' | 'carousel' | 'engagement_metrics' | 'scheduling' | 'dm_automation' | 'content_discovery' | (string & {});
...
export type ChannelEventType = 'message' | 'message_edited' | 'message_deleted' | 'reaction_added' | 'reaction_removed' | 'member_joined' | 'member_left' | 'typing' | 'button_callback' | 'error' | 'connection_change';
/**
...
export type MessageEvent = ChannelEvent<ChannelMessage> & {
    type: 'message';
};
...
export type ButtonCallbackEvent = ChannelEvent<ButtonCallbackData> & {
