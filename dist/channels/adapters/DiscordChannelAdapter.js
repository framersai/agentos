...
export class DiscordChannelAdapter extends BaseChannelAdapter {
    ...
    async doConnect(auth) {
        ...
        if (!botToken) {
            throw new Error('Discord bot token is required. Provide it as credential or params.botToken.');
        }
        // Dynamic import
        ...
        try {
            ...
            throw new Error('The "discord.js" package is required for the Discord adapter. ' +
        ...
        const { GatewayIntentBits, Client, Partials, } = discordJs;
        const intentFlags = [
        ...
        console.log(`[Discord] Connected as ${user?.tag ?? 'unknown'} (${this.client.guilds?.cache?.size ?? 0} guilds)`);
    }
    ...
    wireEventHandlers() {
        ...
        // Message edits
        this.client.on('messageUpdate', (_old, updated) => {
            if (updated.author?.bot && updated.author?.id === this.client?.user?.id)
                return;
            this.emit({
                type: 'message_edited',
                platform: 'discord',
    ...
    buildMessagePayload(content) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const payload = {};
        const embeds = []; // eslint-disable-line @typescript-eslint/no-explicit-any
        const files = []; // eslint-disable-line @typescript-eslint/no-explicit-any
        const components = []; // eslint-disable-line @typescript-eslint/no-explicit-any
        for (const block of content.blocks) {
            switch (block.type) {
                ...
                case 'document': {
                    files.push({
                        ...
                        name: block.type === 'document'
                            ? block.filename ?? 'file' // eslint-disable-line @typescript-eslint/no-explicit-any
                ...
                default:
                    ...
                    console.warn(`[Discord] Unsupported content block type: ${block.type}`); // eslint-disable-line @typescript-eslint/no-explicit-any
                    break;
            }
        }
        if (embeds.length > 0)
...
//# sourceMappingURL=DiscordChannelAdapter.js.map