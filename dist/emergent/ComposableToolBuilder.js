...
export class ComposableToolBuilder {
     ...
     */
    constructor(executeTool, options) {
 ...
 */
function resolveMapping(mapping, ctx, strict = false) {
    const resolved = {};
 ...
 */
function resolveValue(value, ctx, strict = false) {
    if (typeof value === 'string' && value.startsWith('$')) {
        return resolveExpression(value, ctx, strict);
    }
    if (isPlainObject(value)) {
        return resolveMapping(value, ctx, strict);
    }
    return value;
}
/**
 ...
 */
function resolveExpression(expr, ctx, strict = false) {
    // --- $input or $input.path ---
    if (expr === '$input') {
        return ctx.input;
    }
    ...
    if (strict) {
        throw new Error(`Unresolved reference expression: "${expr}". Available: $input, $prev, $steps[N]`);
    }
