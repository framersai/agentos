...
CREATE TABLE IF NOT EXISTS agentos_emergent_audit_log (
  ...
  timestamp BIGINT NOT NULL
    ...
    async persistToolToDb(tool, approvedBy) {
        ...
        let promotedAt = null;
        let promotedBy = approvedBy ?? null;
        if (tool.tier !== 'session') {
            const existing = (await this.db.get(`SELECT promoted_at, promoted_by
             FROM agentos_emergent_tools
            WHERE id = ?
            LIMIT 1`, [tool.id])) ?? { promoted_at: null, promoted_by: null };
            promotedAt =
                approvedBy != null
                    ? Date.now()
        ...
        const implementationSource = tool.implementation.mode === 'sandbox'
    ...
    serializeSandboxImplementation(tool) {
