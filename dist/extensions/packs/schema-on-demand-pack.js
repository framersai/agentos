...
async function getCuratedExtensionsCatalog() {
    const mod = await tryLoadExtensionsRegistry();
    if (!mod || typeof mod.getAvailableExtensions !== 'function') {
        return {
            curated: undefined,
            note: `Curated extension catalog unavailable. Install ${EXT_REGISTRY_MODULE} to enable curated discovery.`,
        };
    }
    try {
        const entries = (await mod.getAvailableExtensions());
...
function listActiveToolNames(manager) {
    return manager
        .getRegistry(EXTENSION_KIND_TOOL)
        .listActive()
        .map((d) => String(d?.id))
        .filter(Boolean);
}
...
export function createSchemaOnDemandPack(opts) {
    ...
    const allowPackages = typeof options.allowPackages === 'boolean' ? options.allowPackages : process.env.NODE_ENV !== 'production';
    const allowModules = options.allowModules === true;
    const officialRegistryOnly = options.officialRegistryOnly !== false;
    const extensionsListTool = {
    ...
    const extensionsEnableTool = {
        id: 'agentos-extensions-enable-v1',
        name: 'extensions_enable',
        displayName: 'Enable Extension',
        description: 'Load an extension pack at runtime so its tools become available in the next tool-listing iteration. This has side effects and should be human-approved.',
        category: 'system',
        hasSideEffects: true,
        inputSchema: {
            type: 'object',
            ...
            properties: {
                extension: {
                    type: 'string',
                    description: "Extension ref. Default source='curated' uses curated names like 'web-search' or 'skills'.",
                },
                source: {
                    type: 'string',
...
//# sourceMappingURL=schema-on-demand-pack.js.map