...
export class Marketplace {
    ...
    async getItems(itemIds) {
        return itemIds.map(id => this.items.get(id)).filter((item) => !!item);
    }
    async getFeatured(type, limit = 10) {
    ...
    async getTrending(type, _period = 'week', limit = 10) {
        let items = Array.from(this.items.values()).filter(i => i.status === 'published');
        if (type)
            items = items.filter(i => i.type === type);
    ...
    async getByPublisher(publisherId, options) {
        return this.search({ ...options, publisherId });
    }
    // ============================================================================
    // Item Details
    // ============================================================================
    async getReviews(itemId, options) {
        const reviewIds = this.reviewsByItem.get(itemId) || new Set();
        const reviews = Array.from(reviewIds).map(id => this.reviews.get(id)).filter((r) => !!r);
    ...
    async getDependencyTree(itemId) {
        const item = this.items.get(itemId);
        if (!item)
    ...
    async update(installationId, options) {
    ...
    async publishVersion(itemId, version, _options) {
        const item = this.items.get(itemId);
        if (!item)
            throw new Error(`Item not found: ${itemId}`);
    ...
    async deprecate(itemId, reason, alternativeId) {
        const item = this.items.get(itemId);
        if (!item)
            throw new Error(`Item not found: ${itemId}`);
    ...
    // Reviews & Ratings
    // ============================================================================
    async submitReview(itemId, review) {
        const item = this.items.get(itemId);
        if (!item)
            throw new Error(`Item not found: ${itemId}`);
        const reviewRecord = {
    ...
    async getItemAnalytics(itemId, _period) {
        const item = this.items.get(itemId);
        if (!item)
            throw new Error(`Item not found: ${itemId}`);
    ...
    async seedSampleItems() {
        ...
        const samplePricing = { model: 'free' };
        const sampleItems = [
            {
                type: 'agent',
