...
export class KnowledgeGraph {
    ...
    // Entity Operations
    // ============================================================================
    async upsertEntity(entityInput) {
    ...
    // Relation Operations
    // ============================================================================
    async upsertRelation(relationInput) {
        const now = new Date().toISOString();
        const relation = {
            ...relationInput,
    ...
    // ============================================================================
    async recordMemory(memoryInput) {
        const now = new Date().toISOString();
        const memory = {
            ...memoryInput,
            id: `mem-${uuidv4()}`,
        ...
        for (let depth = 1; depth <= maxDepth && currentLevel.length > 0; depth++) {
            ...
            for (const entityId of currentLevel) {
                ...
                for (const rel of relations) {
                    if (relationTypes && !relationTypes.has(rel.type))
                        continue;
    ...
    async getNeighborhood(entityId, depth = 1) {
    ...
    async semanticSearch(options) {
        ...
        if (scope === 'entities' || scope === 'all') {
            for (const entity of this.entities.values()) {
                if (options.ownerId && entity.ownerId !== options.ownerId)
                    continue;
