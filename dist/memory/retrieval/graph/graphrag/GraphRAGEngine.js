...
async function resolveGraph() {
    ...
    try {
        ...
        throw new Error('graphology is required for GraphRAGEngine but was not found. ' +
            'Install it: npm install graphology graphology-types');
    }
}
async function resolveLouvain() {
    if (_louvain)
        return _louvain;
    try {
        ...
        throw new Error('graphology-communities-louvain is required for community detection but was not found. ' +
...
export class GraphRAGEngine {
    constructor(deps) {
        ...
        this.ingestedDocumentIds = new Set();
        this.ingestedDocumentHashes = new Map();
        this.documentEntityContributions = new Map();
        this.documentRelationshipContributions = new Map();
        this.tablePrefix = 'graphrag_';
        if (deps) {
    ...
    async resolveEmbeddingDimension() {
    ...
    async hashDocumentContent(content) {
        const text = content ?? '';
    ...
    ensureInitialized() {
        if (!this.isInitialized) {
            throw new GMIError('GraphRAGEngine is not initialized. Call initialize() first.', GMIErrorCode.NOT_INITIALIZED, undefined, 'GraphRAGEngine');
        }
    }
    // ===========================================================================
    ...
    async ingestDocuments(documents) {
        ...
        const entitiesNeedingEmbedding = new Set();
        ...
        for (const doc of documents) {
            ...
            if (wasIngested) {
                try {
                    ...
                    for (const id of removed.touchedEntityIds)
                        touchedEntityIds.add(id);
                    for (const id of removed.touchedRelationshipIds)
                        touchedRelationshipIds.add(id);
                    for (const id of removed.entitiesNeedingEmbedding)
                        entitiesNeedingEmbedding.add(id);
                }
                catch (error) {
                    // Safety: do not double-count if we cannot reliably subtract the previous doc's contributions.
                    console.warn(`[GraphRAGEngine] Skipping update for document '${doc.id}' because previous contribution records are missing. ` +
            ...
            const extractedToCanonicalEntityId = new Map();
            const docEntityContribs = new Map();
            for (const entity of extraction.entities) {
                ...
                if (prev) {
                    ...
                    if (!prev.description && entity.description)
            ...
            for (const rel of extraction.relationships) {
                ...
                const docRelContribs = this.documentRelationshipContributions.get(doc.id) ?? new Map();
                const weight = Number.isFinite(rel.weight) ? rel.weight : 1.0;
                const prev = docRelContribs.get(canonicalRel.id);
                if (prev) {
                    prev.weight += weight;
                    if (!prev.description && rel.description)
                        prev.description = rel.description;
                }
                else {
    ...
    async removeDocuments(documentIds) {
        ...
        for (const rawId of documentIds) {
            ...
            if (!documentId)
                continue;
            if (!this.ingestedDocumentIds.has(documentId))
                continue;
            try {
                ...
                console.warn(`[GraphRAGEngine] Failed to remove document '${documentId}' contributions: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    ...
    // ===========================================================================
    async extractEntitiesAndRelationships(documentId, content) {
    ...
    async llmExtract(documentId, content) {
...
${content.slice(0, 8000)}`;
    ...
    patternExtract(documentId, content) {
        const now = new Date().toISOString();
        const entities = [];
        const relationships = [];
        ...
        for (const sentence of sentences) {
            const sentenceEntities = entities.filter(e => sentence.toLowerCase().includes(e.name.toLowerCase()));
    ...
    extractSentenceContext(text, term) {
    ...
    mergeRelationship(rel) {
        ...
        if (existing) {
            ...
            try {
                this.graph.setEdgeAttribute?.(existing.id, 'weight', existing.weight);
            }
            catch {
            ...
            try {
                this.graph.addEdgeWithKey?.(rel.id, rel.sourceEntityId, rel.targetEntityId, { id: rel.id, type: rel.type, weight: rel.weight });
                if (!this.graph.addEdgeWithKey) {
    ...
    // Entity Embeddings
    // ===========================================================================
    async generateEntityEmbeddings(forceEntityIds) {
    ...
    async detectCommunities() {
        ...
        for (const [communityIdx, entityIds] of communityGroups) {
            if (entityIds.length < (this.config.minCommunitySize ?? 2))
                continue;
    ...
    async buildCommunityHierarchy() {
        ...
        for (let level = 1; level < maxLevels; level++) {
            ...
            if (prevLevelCommunities.length <= 1)
                break;
            ...
            if (metaGraph.order < 2 || metaGraph.size === 0)
                break;
    ...
    async summarizeCommunity(community) {
        if (!this.llmProvider)
            return '';
...
Respond with a clear, informative summary of what this group represents and its significance.`;
    ...
    async globalSearch(query, options) {
        ...
        if (this.embeddingManager && this.vectorStore) {
            ...
            if (queryEmbedding) {
                const searchResult = await this.vectorStore.query(this.config.communityCollectionName, queryEmbedding, {
                    topK: topK * 2,
        ...
        if (this.llmProvider && matchedCommunities.length > 0) {
            ...
            answer = await this.llmProvider.generateText(`Based on the following community summaries, answer the question: "${query}"
...
Provide a comprehensive answer based on the information above.`, { maxTokens: 500, temperature: 0 });
    ...
    async localSearch(query, options) {
        ...
        if (this.embeddingManager && this.vectorStore) {
            ...
            const queryEmbedding = queryEmbedResult.embeddings[0];
            if (queryEmbedding) {
                const searchResult = await this.vectorStore.query(this.config.entityCollectionName, queryEmbedding, {
                    topK: topK * 2,
                    includeTextContent: true,
                    includeMetadata: true,
                    minSimilarityScore: options?.minRelevance,
    ...
    async getRelationships(entityId) {
        this.ensureInitialized();
        return Array.from(this.relationships.values()).filter(r => r.sourceEntityId === entityId || r.targetEntityId === entityId);
    }
    ...
    `);
        ...
        try {
            await this.persistenceAdapter.exec(`ALTER TABLE ${this.tablePrefix}ingested_documents ADD COLUMN content_hash TEXT;`);
        }
        catch {
    ...
    async loadFromPersistence() {
        if (!this.persistenceAdapter)
            return;
        // Load entities
        const entityRows = await this.persistenceAdapter.all(`SELECT * FROM ${this.tablePrefix}entities`);
        ...
        // Load relationships
        const relRows = await this.persistenceAdapter.all(`SELECT * FROM ${this.tablePrefix}relationships`);
        ...
        // Load communities
        const commRows = await this.persistenceAdapter.all(`SELECT * FROM ${this.tablePrefix}communities`);
        ...
        try {
            const docRows = await this.persistenceAdapter.all(`SELECT document_id, content_hash FROM ${this.tablePrefix}ingested_documents`);
            ...
            const docRows = await this.persistenceAdapter.all(`SELECT document_id FROM ${this.tablePrefix}ingested_documents`);
            for (const row of docRows) {
                this.ingestedDocumentIds.add(row.document_id);
            }
        }
        ...
        try {
            const docEntityRows = await this.persistenceAdapter.all(`SELECT document_id, entity_id, entity_name, entity_type, description, frequency FROM ${this.tablePrefix}document_entities`);
            for (const row of docEntityRows) {
                const docId = String(row.document_id);
                const entityId = String(row.entity_id);
                const map = this.documentEntityContributions.get(docId) ?? new Map();
                map.set(entityId, {
                    entityId,
            ...
            const docRelRows = await this.persistenceAdapter.all(`SELECT document_id, relationship_id, source_entity_id, target_entity_id, type, description, weight FROM ${this.tablePrefix}document_relationships`);
            for (const row of docRelRows) {
                const docId = String(row.document_id);
                const relationshipId = String(row.relationship_id);
                const map = this.documentRelationshipContributions.get(docId) ??
                    new Map();
    ...
    `);
        ...
        for (const docId of this.ingestedDocumentIds) {
            await this.persistenceAdapter.run(`INSERT INTO ${this.tablePrefix}ingested_documents (document_id, ingested_at, content_hash) VALUES (?, ?, ?)`, [docId, new Date().toISOString(), this.ingestedDocumentHashes.get(docId) ?? null]);
        }
        // Persist entities
        for (const entity of this.entities.values()) {
            await this.persistenceAdapter.run(`INSERT OR REPLACE INTO ${this.tablePrefix}entities
        ...
        for (const rel of this.relationships.values()) {
            await this.persistenceAdapter.run(`INSERT OR REPLACE INTO ${this.tablePrefix}relationships
        ...
        // Persist communities
        for (const community of this.communities.values()) {
            await this.persistenceAdapter.run(`INSERT OR REPLACE INTO ${this.tablePrefix}communities
        ...
        for (const [docId, entityMap] of this.documentEntityContributions) {
            for (const contrib of entityMap.values()) {
                await this.persistenceAdapter.run(`INSERT INTO ${this.tablePrefix}document_entities (document_id, entity_id, entity_name, entity_type, description, frequency)
           VALUES (?, ?, ?, ?, ?, ?)`, [
                    docId,
                    contrib.entityId,
                    contrib.name ?? null,
        ...
        for (const [docId, relMap] of this.documentRelationshipContributions) {
            for (const contrib of relMap.values()) {
                await this.persistenceAdapter.run(`INSERT INTO ${this.tablePrefix}document_relationships (document_id, relationship_id, source_entity_id, target_entity_id, type, description, weight)
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                    docId,
    ...
    recomputeEntityAggregates(entityId) {
    ...
    recomputeRelationshipAggregates(relationshipId) {
        ...
        try {
            this.graph.setEdgeAttribute?.(relationshipId, 'weight', rel.weight);
        }
        catch {
            // ignore
        }
    }
    async hydrateDocumentContributionsFromPersistence(documentId) {
        if (!this.persistenceAdapter)
            return;
        if (!this.documentEntityContributions.has(documentId)) {
            try {
                const rows = await this.persistenceAdapter.all(`SELECT entity_id, entity_name, entity_type, description, frequency FROM ${this.tablePrefix}document_entities WHERE document_id = ?`, [documentId]);
                const map = new Map();
        ...
        if (!this.documentRelationshipContributions.has(documentId)) {
            try {
                const rows = await this.persistenceAdapter.all(`SELECT relationship_id, source_entity_id, target_entity_id, type, description, weight FROM ${this.tablePrefix}document_relationships WHERE document_id = ?`, [documentId]);
                const map = new Map();
                for (const row of rows) {
                    const relationshipId = String(row.relationship_id);
                    map.set(relationshipId, {
    ...
    async removeDocumentContributions(documentId) {
        ...
        if (relContribs) {
            for (const contrib of relContribs.values()) {
                ...
                if (rel.weight <= 0 || rel.sourceDocumentIds.length === 0) {
                    ...
                    try {
                        this.graph.setEdgeAttribute?.(rel.id, 'weight', rel.weight);
                    }
                    catch {
                        // ignore
                    }
