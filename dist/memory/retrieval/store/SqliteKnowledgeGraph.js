...
export class SqliteKnowledgeGraph {
     ...
     */
    async getEntity(id) {
        const row = await this.brain.get('SELECT * FROM knowledge_nodes WHERE id = ?', [id]);
        if (!row)
            return undefined;
    ...
    async queryEntities(options) {
        ...
        const rows = await this.brain.all(sql, params);
    ...
    async deleteEntity(id) {
        return await this.brain.transaction(async (trx) => {
            ...
            await trx.run('DELETE FROM knowledge_edges WHERE source_id = ? OR target_id = ?', [id, id]);
            // Delete the node itself.
            const result = await trx.run('DELETE FROM knowledge_nodes WHERE id = ?', [id]);
    ...
    async deleteRelation(id) {
        const result = await this.brain.run('DELETE FROM knowledge_edges WHERE id = ?', [id]);
        return result.changes > 0;
    }
    // =========================================================================
    ...
    async recordMemory(memory) {
        ...
        await this.brain.run(`INSERT INTO knowledge_nodes
     ...
     */
    async getMemory(id) {
        const row = await this.brain.get(`SELECT * FROM knowledge_nodes WHERE id = ? AND type = 'memory'`, [id]);
        if (!row)
            return undefined;
    ...
    `;
        ...
        const traversalRows = await this.brain.all(sql, params);
        ...
        for (const [depth, data] of [...levelMap.entries()].sort((a, b) => a[0] - b[0])) {
            ...
            for (const rid of data.edgeIds) {
                const row = await this.brain.get('SELECT * FROM knowledge_edges WHERE id = ?', [rid]);
                if (row)
    ...
    async findPath(sourceId, targetId, maxDepth) {
      ...
      LIMIT 1
    `;
        const row = await this.brain.get(sql, [sourceId, sourceId, depth, targetId]);
        if (!row)
            return null;
        ...
        for (let i = 0; i < entityIds.length; i++) {
            ...
            let relation;
            if (i > 0 && edgeIds[i - 1]) {
                const edgeRow = await this.brain.get('SELECT * FROM knowledge_edges WHERE id = ?', [edgeIds[i - 1]]);
                if (edgeRow)
    ...
    `;
        const rows = await this.brain.all(sql, [entityId, maxDepth]);
        ...
        const allEdges = await this.brain.all('SELECT * FROM knowledge_edges');
    ...
    async semanticSearch(options) {
        ...
        if (results.length === 0) {
            ...
            const allRows = await this.brain.all(allNodesSql);
    ...
    async mergeEntities(entityIds, primaryId) {
        ...
        await this.brain.transaction(async (trx) => {
            for (const otherId of othersIds) {
                ...
                await trx.run('UPDATE knowledge_edges SET source_id = ? WHERE source_id = ?', [primaryId, otherId]);
                // Re-link incoming edges: target_id = otherId -> primaryId.
                await trx.run('UPDATE knowledge_edges SET target_id = ? WHERE target_id = ?', [primaryId, otherId]);
                // Delete the non-primary entity node.
                await trx.run('DELETE FROM knowledge_nodes WHERE id = ?', [otherId]);
            }
            // Remove any self-referential edges that may have resulted from the merge.
            await trx.run('DELETE FROM knowledge_edges WHERE source_id = target_id');
        });
    ...
    async decayMemories(decayFactor) {
        ...
        const result = await this.brain.run(`UPDATE knowledge_nodes
       SET confidence = confidence * ?
       WHERE type = 'memory'`, [factor]);
    ...
    async getStats() {
        ...
        const entityCountRow = await this.brain.get("SELECT COUNT(*) AS cnt FROM knowledge_nodes WHERE type != 'memory'");
        const entityCount = entityCountRow?.cnt ?? 0;
        // Total relations.
        const relationCountRow = await this.brain.get('SELECT COUNT(*) AS cnt FROM knowledge_edges');
        const relationCount = relationCountRow?.cnt ?? 0;
        // Total memories.
        const memoryCountRow = await this.brain.get("SELECT COUNT(*) AS cnt FROM knowledge_nodes WHERE type = 'memory'");
        const memoryCount = memoryCountRow?.cnt ?? 0;
        // Entity types breakdown (exclude memories).
        const entityTypeRows = await this.brain.all("SELECT type, COUNT(*) AS cnt FROM knowledge_nodes WHERE type != 'memory' GROUP BY type");
        const entitiesByType = {};
        for (const row of entityTypeRows) {
            entitiesByType[row.type] = row.cnt;
        }
        // Relation types breakdown.
        const relTypeRows = await this.brain.all('SELECT type, COUNT(*) AS cnt FROM knowledge_edges GROUP BY type');
        const relationsByType = {};
        for (const row of relTypeRows) {
            relationsByType[row.type] = row.cnt;
        }
        // Average confidence across all nodes.
        const avgConfRow = await this.brain.get('SELECT AVG(confidence) AS avg_conf FROM knowledge_nodes');
        const avgConf = avgConfRow?.avg_conf ?? 0;
        // Oldest and newest entries.
        const oldest = await this.brain.get('SELECT created_at FROM knowledge_nodes ORDER BY created_at ASC LIMIT 1');
        const newest = await this.brain.get('SELECT created_at FROM knowledge_nodes ORDER BY created_at DESC LIMIT 1');
        return {
