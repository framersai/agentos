...
import { parseTraceMetadata, readPersistedDecayState, withPersistedDecayState, } from '../../retrieval/store/tracePersistence.js';
// ---------------------------------------------------------------------------
...
export class RetrievalFeedbackSignal {
     ...
     */
    async detect(injectedTraces, response, context) {
        const responseLower = response.toLowerCase();
        const feedbacks = [];
        const now = Date.now();
        const insertSql = `INSERT INTO retrieval_feedback (trace_id, signal, query, created_at)
       VALUES (?, ?, ?, ?)`;
