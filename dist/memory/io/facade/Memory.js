...
async function _getPath() {
    if (!_isNode)
        throw new Error('Path operations are not available in browser environments.');
    return import('node:path');
}
async function _getOs() {
    if (!_isNode)
...
import { evaluateRetrievalConfidence, getCandidateLimit, resolveMemoryRetrievalPolicy, } from '../../../rag/unified/index.js';
 ...
 */
function standaloneRecallConfidence(score) {
...
export class Memory {
    ...
    async remember(content, options) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, NULL, 0, ?, ?, ?, 0)`, [
        ...
        try {
            await this._brain.run(fts.syncInsert('memory_traces_fts', '(SELECT rowid FROM memory_traces WHERE id = ?)', ['content', 'tags']), [id, content, JSON.stringify(tags)]);
        }
        catch (error) {
    ...
    async recall(query, options) {
        ...
        `;
                let ftsRows = [];
                try {
                    ftsRows = await this._brain.all(ftsSql, [...params, ftsQuery, candidateLimit]);
                }
                catch (error) {
                    if (!isFtsUnavailableError(error)) {
                        throw error;
                ...
                if (topIds.length > 0) {
                    ...
                    const confidence = evaluateRetrievalConfidence(results.map((row) => ({
    ...
    `;
        ...
        let rows;
        try {
            rows = await this._brain.all(sql, params);
        }
        catch (error) {
            if (!isFtsUnavailableError(error)) {
                throw error;
     ...
     */
    async feedback(traceId, signal, query) {
     ...
     */
    async feedbackFromResponse(injectedTraces, response, query) {
        if (!this._feedbackSignal)
            return [];
     ...
     * @returns Summary of the import operation.
     */
    async importFromString(content, format, options) {
        await this._initPromise;
        let result;
        if (format === 'json') {
            const { JsonImporter } = await import('../JsonImporter.js');
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        for (const chunk of chunks) {
         ...
         VALUES (?, 'semantic', 'user', ?, NULL, 1.0, ?, NULL, 0, '[]', '{}', ?, 0)`, [
            ...
            await this._brain.run(this._brain.features.fts.syncInsert('memory_traces_fts', '(SELECT rowid FROM memory_traces WHERE id = ?)', ['content', 'tags']), [traceId, chunk.content, '[]']).catch((error) => {
                if (!isFtsUnavailableError(error)) {
                    throw error;
                }
            });
    ...
    async _recallWithoutFts(rawQuery, whereClause, baseParams, limit) {
      ...
      LIMIT ?
    `;
        return this._brain.all(sql, [...baseParams, ...likeTerms, ...likeTerms, ...likeTerms, limit]);
    }
    /**
     ...
     */
    async _rebuildHnswIndex() {
        if (!this._hnswSidecar)
            return;
        try {
            const rows = await this._brain.all('SELECT id, embedding FROM memory_traces WHERE deleted = 0 AND embedding IS NOT NULL');
            if (rows.length === 0)
                return;
            const { blobCodec } = this._brain.features;
            const items = rows
     ...
     */
    async _detectExportFormat(outputPath, options) {
        if (options?.format)
