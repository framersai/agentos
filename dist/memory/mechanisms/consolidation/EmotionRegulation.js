 ...
 */
export function applyEmotionRegulation(traces, config) {
    if (!config.enabled)
        return 0;
    // Identify regulation candidates
    const candidates = traces.filter((t) => t.isActive &&
    ...
    for (const trace of candidates) {
        if (regulatedCount >= config.maxRegulationPerCycle)
            break;
        ...
        if (!meta.reappraisalHistory)
            meta.reappraisalHistory = [];
    ...
    for (const trace of candidates) {
        if (Math.abs(trace.emotionalContext.valence) <= config.suppressionThreshold)
            continue;
        if (trace.emotionalContext.arousal <= MIN_AROUSAL_FOR_REGULATION)
            continue;
...
//# sourceMappingURL=EmotionRegulation.js.map