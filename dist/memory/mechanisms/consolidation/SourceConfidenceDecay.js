 ...
 */
export function applySourceConfidenceDecay(traces, config) {
    if (!config.enabled)
        return 0;
    const now = Date.now();
    let count = 0;
    for (const trace of traces) {
        ...
        if (meta.lastSourceDecayAt && (now - meta.lastSourceDecayAt) < 3600000)
            continue;
...
//# sourceMappingURL=SourceConfidenceDecay.js.map