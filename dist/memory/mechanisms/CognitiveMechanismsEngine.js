 ...
 */
function applyPersonalityModulation(cfg, traits) {
    if (!traits)
        return cfg;
...
export class CognitiveMechanismsEngine {
     ...
     */
    onRetrieval(results, allCandidates, retrievalCutoff, queryEntities) {
        ...
        const competitors = allCandidates
            ...
            .map((c) => c.trace);
        const { suppressedIds } = applyRetrievalInducedForgetting(results, competitors, this.cfg.retrievalInducedForgetting);
        // FOK: detect partially activated traces
        // Build approximate ScoredMemoryTrace for non-retrieved candidates
        const allScored = allCandidates.map((c) => {
        ...
        const metacognitiveSignals = detectFeelingOfKnowing(allScored, retrievalCutoff, this.cfg.metacognitiveFOK, queryEntities);
     ...
     */
    onEncoding(trace, traceEmbedding) {
     ...
     */
    async onConsolidation(traces, llmFn) {
     ...
     */
    onPromptAssembly(allTraces, retrievedIds) {
        const involuntaryMemory = selectInvoluntaryMemory(allTraces, retrievedIds, this.cfg.involuntaryRecall);
...
//# sourceMappingURL=CognitiveMechanismsEngine.js.map