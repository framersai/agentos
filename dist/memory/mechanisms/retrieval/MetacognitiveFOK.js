 ...
 */
export function detectFeelingOfKnowing(scoredCandidates, retrievalCutoff, config, queryEntities = []) {
    if (!config.enabled)
        return [];
    const signals = [];
    ...
    for (const trace of scoredCandidates) {
        ...
        if (score >= retrievalCutoff || score < config.partialActivationThreshold)
            continue;
        ...
        if (trace.entities.length > 0)
            parts.push(trace.entities.slice(0, 3).join(', '));
...
//# sourceMappingURL=MetacognitiveFOK.js.map