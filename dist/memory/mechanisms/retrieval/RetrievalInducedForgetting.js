 ...
 */
export function applyRetrievalInducedForgetting(retrieved, competitors, config) {
    if (!config.enabled)
    ...
    for (const comp of competitors) {
        if (count >= config.maxSuppressionsPerQuery)
            break;
        if (retrievedIds.has(comp.id))
            continue;
        // Guard: never suppress flashbulb-grade traces
        if (comp.encodingStrength >= FLASHBULB_STRENGTH_FLOOR)
            continue;
        // Guard: skip nearly-dead traces
        if (comp.encodingStrength < MIN_SUPPRESSION_STRENGTH)
            continue;
...
//# sourceMappingURL=RetrievalInducedForgetting.js.map