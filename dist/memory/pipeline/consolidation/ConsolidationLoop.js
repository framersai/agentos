...
import { buildInitialTraceMetadata, parseTraceMetadata, readPersistedDecayState, withPersistedDecayState, } from '../../retrieval/store/tracePersistence.js';
// ---------------------------------------------------------------------------
// Defaults
// ---------------------------------------------------------------------------
...
export class ConsolidationLoop {
    ...
    async _mergeTracePair(a, b, deletedIds) {
        ...
        const mergedLastAccessed = Math.max(survivor.last_accessed ?? survivor.created_at, loser.last_accessed ?? loser.created_at);
        const mergedMetadata = JSON.stringify(withPersistedDecayState(survivorMetadata, {
            ...
            reinforcementInterval: Math.max(survivorDecay.reinforcementInterval, loserDecay.reinforcementInterval),
            ...((survivorDecay.nextReinforcementAt !== undefined ||
                loserDecay.nextReinforcementAt !== undefined)
                ? {
                    nextReinforcementAt: Math.max(survivorDecay.nextReinforcementAt ?? 0, loserDecay.nextReinforcementAt ?? 0),
                }
                : {}),
        }));
        // Update survivor.
    ...
    _rowToMinimalTrace(row) {
        ...
        return {
            ...
            entities: Array.isArray(metadata.entities)
                ? metadata.entities.filter((entity) => typeof entity === 'string')
                : [],
            tags: [],
            provenance: {
