...
const parseDurationToMs = (durationStr) => {
    ...
    if (s.startsWith('pt')) {
        let totalMs = 0;
        const hourMatchIso = s.match(/(\d+)h/);
        if (hourMatchIso)
            totalMs += parseInt(hourMatchIso[1], 10) * 3600 * 1000;
        const minuteMatchIso = s.match(/(\d+)m/);
        if (minuteMatchIso)
            totalMs += parseInt(minuteMatchIso[1], 10) * 60 * 1000;
        const secondMatchIso = s.match(/(\d+)s/);
        if (secondMatchIso)
            totalMs += parseInt(secondMatchIso[1], 10) * 1000;
        if (totalMs > 0)
            return totalMs;
    }
...
export class MemoryLifecycleManager {
     ...
     * @inheritdoc
     */
    async initialize(config, vectorStoreManager, gmiResolver, utilityAI) {
        ...
        if (!config)
            throw new GMIError('MemoryLifecycleManagerConfig cannot be null.', GMIErrorCode.CONFIG_ERROR, { managerId: this.managerId });
        if (!vectorStoreManager)
            throw new GMIError('IVectorStoreManager dependency is missing for MemoryLifecycleManager.', GMIErrorCode.DEPENDENCY_ERROR, { managerId: this.managerId });
        if (!gmiResolver)
            throw new GMIError('GMIResolverFunction dependency is missing for MemoryLifecycleManager.', GMIErrorCode.DEPENDENCY_ERROR, { managerId: this.managerId });
        this.config = {
    ...
    async enforcePolicies(filter) {
        ...
        const policiesToEvaluate = this.config.policies.filter(p => p.isEnabled !== false &&
            (!filter?.policyIds || filter.policyIds.length === 0 || filter.policyIds.includes(p.policyId))).sort((a, b) => (a.priority || 0) - (b.priority || 0));
     ...
     * @private
     */
    async findPolicyCandidates(policy, enforcementFilter) {
        ...
        for (const dsId of dsIdsToScan) {
            try {
                ...
                const combinedFilter = { ...(policy.appliesTo.metadataFilter || {}) };
                ...
                if (targetCategories && targetCategories.length > 0) {
                    combinedFilter['category'] = { $in: targetCategories };
                }
    ...
    async negotiateAndDetermineAction(candidate, policy) {
        if (policy.gmiNegotiation?.enabled && candidate.gmiOwnerId) {
            ...
            if (gmiInstance) {
                const event = {
                    ...
                    proposedAction: policy.action.type, // Map from ConfigPolicyActionDetails['type']
                ...
                try {
                    ...
                    const response = await Promise.race([
                        negotiationPromise,
                        new Promise((_, reject) => setTimeout(() => reject(new GMIError("GMI negotiation timed out", GMIErrorCode.TIMEOUT, { itemId: candidate.id })), timeout))
                    ]);
        ...
        return policy.action.type; // No negotiation configured
    }
    mapPolicyActionToEventType(actionType) {
    ...
    async executeLifecycleAction(candidate, configuredActionDetails, determinedAction, // This is the action post-negotiation
    policyId, report // For adding detailed traces
    ) {
        /**
        ...
        if (determinedAction === 'ALLOW_ACTION') {
            ...
            effectiveConfigActionType = determinedAction;
        }
        try {
            let summaryText;
            if (effectiveConfigActionType.startsWith('summarize_')) {
                ...
                if (!candidate.textContent) {
                    ...
                    this.addTraceToReport(report, candidate.id, policyId, determinedAction, `Text content missing for summarization; skipping summary generation.`);
                    console.warn(`MLM (${this.managerId}): Text content missing for item '${candidate.id}' during summarize action. Skipping summary.`);
                }
                else {
                    ...
                    if (configuredActionDetails.summaryDataSourceId && summaryText) {
                        ...
                        this.addTraceToReport(report, candidate.id, policyId, determinedAction, `Summary (New ID: ${summaryDocId}) for item '${candidate.id}' to be ingested to DS '${configuredActionDetails.summaryDataSourceId}'. (Conceptual RAG ingestion step).`);
            ...
            if (effectiveConfigActionType === 'delete' || (effectiveConfigActionType === 'summarize_and_delete' && (configuredActionDetails.deleteOriginalAfterSummary !== false || summaryText !== undefined))) {
                ...
                if (this.archive) {
                    ...
                    const writeResult = await this.archive.store({
                        ...
                        contentHash,
                        traceType: candidate.category ?? 'episodic',
                    ...
                    if (!writeResult.success) {
                        this.addTraceToReport(report, candidate.id, policyId, determinedAction, `ARCHIVAL_FAILED: ${writeResult.error}. Original retained.`);
                        return;
                    }
                    this.addTraceToReport(report, candidate.id, policyId, determinedAction, `Archived (${writeResult.bytesWritten} bytes). Deleting original.`);
                }
                else {
                    ...
                    this.addTraceToReport(report, candidate.id, policyId, determinedAction, `No archive configured (target '${archiveTarget}' is conceptual). Falling back to delete.`);
                }
    ...
    async processSingleItemLifecycle(itemContext, triggeringReason) {
        this.ensureInitialized();
        ...
        const applicablePolicies = this.config.policies.filter(p => {
            ...
            let matches = true;
            if (p.appliesTo.categories && !p.appliesTo.categories.some(cat => cat === candidate.category))
                matches = false;
            if (p.appliesTo.dataSourceIds && !p.appliesTo.dataSourceIds.includes(candidate.dataSourceId))
                matches = false;
            if (p.appliesTo.gmiOwnerId && p.appliesTo.gmiOwnerId !== candidate.gmiOwnerId)
                matches = false;
            if (p.appliesTo.personaOwnerId && p.appliesTo.personaOwnerId !== candidate.personaOwnerId)
                matches = false;
            if (p.appliesTo.metadataFilter && candidate.metadata) {
                const filter = p.appliesTo.metadataFilter;
                const meta = candidate.metadata;
                const filterMatches = Object.entries(filter).every(([key, val]) => meta[key] === val);
                if (!filterMatches)
                    matches = false;
            }
            if (!matches)
            ...
            if (p.minAccessCount && candidate.metadata?.accessCount != null) {
                if (candidate.metadata.accessCount >= p.minAccessCount)
                    return false;
            }
            return true;
        }).sort((a, b) => (b.priority || 0) - (a.priority || 0));
        if (applicablePolicies.length === 0) {
            const withinRetention = candidate.timestamp &&
        ...
        let reportForSingleItem; // Create a mini-report for tracing this one action
    ...
    async checkHealth() {
        ...
        if (!this.isInitialized)
            return { isHealthy: false, details: { message: `MLM (ID: ${this.managerId}) not initialized.` } };
     ...
     * @private
     */
    addTraceToReport(report, itemId, policyId, action, message, details) {
...
//# sourceMappingURL=MemoryLifecycleManager.js.map