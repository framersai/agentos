 ...
 * available.
 */
let _catalogCachePromise;
function getLoadableCatalogNames() {
...
function resolveCandidateExtensionId(capabilityName, sourceRef) {
 ...
 */
export function createDiscoverCapabilitiesTool(discoveryEngine, toolOrchestrator) {
    return {
        ...
        async execute(args, _context) {
            ...
            try {
                ...
                const loadableNames = toolOrchestrator
                    ...
                    : new Set();
                const capabilities = await Promise.all(result.tier1.map(async (r) => {
                    const base = {
                        id: r.capability.id,
                    ...
                    const candidateExtensionId = resolveCandidateExtensionId(capName, sourceRef);
