...
export class CapabilityIndex {
    ...
    async search(query, topK, filters) {
        ...
        const normalizedExcludedCapabilityIds = normalizeExcludedCapabilityIds(filters?.excludedCapabilityIds);
        ...
        // Query vector store
        const queryTopK = normalizedExcludedCapabilityIds.length > 0
            ? Math.min(this.descriptors.size, Math.max(topK, topK + normalizedExcludedCapabilityIds.length * 4))
            : topK;
...
function matchesExcludedCapability(descriptor, normalizedExcludedCapabilityIds) {
