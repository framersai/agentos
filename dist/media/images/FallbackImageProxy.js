 ...
 * failing.
 */
export class FallbackImageProxy {
    /**
    ...
    async generateImage(request) {
        return this.runWithFallback('generateImage', (provider) => provider.generateImage(request));
    }
    // -----------------------------------------------------------------------
    ...
    async editImage(request) {
        return this.runWithFallback('editImage', (provider) => {
            if (typeof provider.editImage !== 'function') {
                throw Object.assign(new Error(`Provider "${provider.providerId}" does not support editImage.`), { name: 'ImageEditNotSupportedError' });
            }
    ...
    async upscaleImage(request) {
        return this.runWithFallback('upscaleImage', (provider) => {
            if (typeof provider.upscaleImage !== 'function') {
                throw Object.assign(new Error(`Provider "${provider.providerId}" does not support upscaleImage.`), { name: 'ImageUpscaleNotSupportedError' });
            }
     ...
     */
    async variateImage(request) {
        return this.runWithFallback('variateImage', (provider) => {
            if (typeof provider.variateImage !== 'function') {
                throw Object.assign(new Error(`Provider "${provider.providerId}" does not support variateImage.`), { name: 'ImageVariationNotSupportedError' });
            }
    ...
    async runWithFallback(operationName, execute) {
        ...
        throw new AggregateError(errors, `All ${this.chain.length} image providers failed for ${operationName}: ${errors.map((e) => e.message).join('; ')}`);
    }
}
//# sourceMappingURL=FallbackImageProxy.js.map