...
function resolveStabilityRoute(modelId, providerOptions) {
...
export class StabilityImageProvider {
    ...
    async generateImage(request) {
        ...
        appendIfDefined(formData, 'aspect_ratio', providerOptions?.aspectRatio ?? request.aspectRatio ?? inferAspectRatioFromSize(request.size));
        appendIfDefined(formData, 'seed', providerOptions?.seed ?? request.seed);
        appendIfDefined(formData, 'output_format', normalizeOutputFormat(providerOptions?.outputFormat ?? request.outputFormat));
    ...
    async editImage(request) {
        ...
        formData.append('image', new Blob([bufferToBlobPart(request.image)], { type: 'image/png' }), 'image.png');
        formData.append('prompt', request.prompt);
        ...
        if (request.mask) {
            formData.append('mask_image', new Blob([bufferToBlobPart(request.mask)], { type: 'image/png' }), 'mask.png');
        }
    ...
    async upscaleImage(request) {
        ...
        if (request.height)
            appendIfDefined(formData, 'height', request.height);
        appendIfDefined(formData, 'output_format', normalizeOutputFormat(providerOptions?.outputFormat));
     ...
     */
    async parseStabilityResponse(response, providerOptions) {
        const contentType = response.headers.get('content-type') ?? '';
        const images = [];
        if (contentType.includes('application/json')) {
            const json = (await response.json());
...
//# sourceMappingURL=StabilityImageProvider.js.map