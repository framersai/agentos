 ...
 */
const exactMatchScorer = (actual, expected) => {
    if (!expected)
        return 0;
 ...
 */
const containsScorer = (actual, expected) => {
    if (!expected)
        return 0;
 ...
 */
const levenshteinScorer = (actual, expected) => {
    if (!expected)
        return 0;
 ...
 */
const wordOverlapScorer = (actual, expected, references) => {
 ...
 */
const bleuScorer = (actual, expected, references) => {
    if (!expected && (!references || references.length === 0))
        return 0;
    ...
    const brevityPenalty = actualTokens.length >= refLen ? 1 : Math.exp(1 - refLen / actualTokens.length);
    return maxPrecision * brevityPenalty;
};
/**
 * ROUGE-L score approximation.
 */
const rougeScorer = (actual, expected) => {
    if (!expected)
        return 0;
    const a = actual.toLowerCase().split(/\s+/);
    const b = expected.toLowerCase().split(/\s+/);
    // Find longest common subsequence
    const lcs = Array(a.length + 1)
        .fill(null)
...
export class Evaluator {
    ...
    async runEvaluation(name, testCases, agentFn, config) {
        const runId = uuidv4();
        ...
        try {
            ...
            const runWithRetry = async (testCase) => {
                let lastError;
                for (let attempt = 0; attempt <= (cfg.retries || 0); attempt++) {
                    try {
                        ...
                        const actualOutput = await Promise.race([
                            ...
                            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), cfg.timeoutMs)),
                        ]);
    ...
    async evaluateTestCase(testCase, actualOutput, config) {
        const cfg = { ...DEFAULT_CONFIG, ...config };
        const metrics = [];
        let totalScore = 0;
        let totalWeight = 0;
        ...
        for (const criterion of criteria) {
            ...
            const score = await scorer(actualOutput, testCase.expectedOutput, testCase.referenceOutputs, testCase.metadata);
            metrics.push({
                name: criterion.name,
                type: 'custom',
                value: score,
    ...
    async score(scorer, actual, expected, references) {
    ...
    async compareRuns(runId1, runId2) {
        ...
        const compareMetric = (name, v1, v2) => {
            const delta = v2 - v1;
    ...
    async generateReport(runId, format) {
