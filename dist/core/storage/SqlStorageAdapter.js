...
export class SqlStorageAdapter {
    ...
    async getConversation(conversationId) {
        this.ensureInitialized();
        const row = await this.adapter.get('SELECT * FROM conversations WHERE id = ?', [conversationId]);
        if (!row) {
            return null;
        }
    ...
    async deleteConversation(conversationId) {
        this.ensureInitialized();
        const result = await this.adapter.run('DELETE FROM conversations WHERE id = ?', [conversationId]);
    ...
    async storeMessage(message) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        await this.adapter.run('UPDATE conversations SET lastActivity = ? WHERE id = ?', [message.timestamp, message.conversationId]);
        return message;
    }
    /**
    ...
    async getMessage(messageId) {
        this.ensureInitialized();
        const row = await this.adapter.get('SELECT * FROM messages WHERE id = ?', [messageId]);
        if (!row) {
            return null;
        }
     ...
     */
    async deleteMessagesForConversation(conversationId) {
        this.ensureInitialized();
    ...
    async getMessageCount(conversationId) {
        this.ensureInitialized();
        const row = await this.adapter.get('SELECT COUNT(*) as count FROM messages WHERE conversationId = ?', [conversationId]);
        return row?.count || 0;
    }
    /**
...
//# sourceMappingURL=SqlStorageAdapter.js.map