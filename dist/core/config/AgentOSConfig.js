 ...
 */
function createPersonaLoaderConfig(env) {
    return {
 ...
 */
function createModelProviderManagerConfig(env) {
    const providers = [];
    // OpenAI Provider
    if (env.OPENAI_API_KEY) {
        providers.push({
 ...
 */
function _createLemonSqueezyService(_env) {
    return {
        initialize: async () => {
            /* implementation */
        },
        verifyWebhookSignature: (_rawBody, _signature) => true,
        processWebhookEvent: async (_eventName, _data) => {
            /* implementation */
        },
    };
}
 ...
 */
async function createUtilityAIService(env) {
 ...
 */
export async function createAgentOSConfig(options = {}) {
    ...
    try {
        ...
        const toolPermissionManagerConfig = {
        ...
        const conversationManagerConfig = {
        ...
        if (env.OPENAI_API_KEY)
            extensionSecrets['openai.apiKey'] = env.OPENAI_API_KEY;
        if (env.OPENROUTER_API_KEY)
            extensionSecrets['openrouter.apiKey'] = env.OPENROUTER_API_KEY;
        if (env.ANTHROPIC_API_KEY)
            extensionSecrets['anthropic.apiKey'] = env.ANTHROPIC_API_KEY;
        if (env.SERPER_API_KEY)
            extensionSecrets['serper.apiKey'] = env.SERPER_API_KEY;
        const config = {
            gmiManagerConfig,
            orchestratorConfig,
            promptEngineConfig,
        ...
        throw new GMIError(`Failed to create AgentOS configuration: ${error.message}`, GMIErrorCode.CONFIGURATION_ERROR, { originalError: error });
    }
}
/**
 ...
 */
export async function createTestAgentOSConfig(options = {}) {
...
//# sourceMappingURL=AgentOSConfig.js.map