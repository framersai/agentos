 ...
 * @see GeminiCLIBridge
 */
import { GeminiCLIBridge } from './GeminiCLIBridge.js';
...
export class GeminiCLIProvider {
    ...
    async initialize(config) {
        ...
        if (!installCheck.installed) {
            throw new GeminiCLIProviderError('Gemini CLI is not installed.', 'BINARY_NOT_FOUND', 'Install Gemini CLI: npm install -g @google/gemini-cli\n\nThen log in by running "gemini" in your terminal.\n\nAlternatively, switch to a different provider:\n  wunderland login', false);
        }
        const isAuth = await this.bridge.checkAuthenticated();
        if (!isAuth) {
            throw new GeminiCLIProviderError('Gemini CLI is installed but not logged in.', 'NOT_AUTHENTICATED', 'Open your terminal and run:\n  gemini\n\nComplete the Google account login flow, then restart your agent.', false);
        }
    ...
    async generateCompletion(modelId, messages, options) {
        ...
        if (result.isError) {
            throw new GeminiCLIProviderError(`Gemini CLI returned an error: ${result.result}`, 'CRASHED', 'Try running "gemini -p test" manually to diagnose.', true);
        }
    ...
    async *generateCompletionStream(modelId, messages, options) {
        ...
        try {
                ...
                switch (event.type) {
                    ...
                    case 'result': {
                        ...
                        if (hasTools) {
                            ...
                            if (toolCalls) {
                                emittedFinal = true;
                                yield this.buildToolCallResponse(toolCalls, { sessionId: event.sessionId, usage: event.usage }, responseId, finalUsage, modelId);
                                return;
                            }
                        }
                        emittedFinal = true;
                    ...
                    case 'error':
                        emittedFinal = true;
                        yield this.buildStreamErrorResponse(`Gemini CLI stream error: ${event.error}`, modelId, responseId, finalUsage);
                        return;
                }
            }
        }
            ...
            emittedFinal = true;
            yield this.buildStreamErrorResponse(error?.message ?? 'Gemini CLI stream failed.', modelId, responseId, finalUsage, error?.code, error);
            return;
        }
        if (!emittedFinal) {
    ...
    async generateEmbeddings(_modelId, _texts, _options) {
        throw new GeminiCLIProviderError('Gemini CLI does not support embeddings. Use a different provider (OpenAI, Ollama, etc.) for embedding operations.', 'EMBEDDINGS_NOT_SUPPORTED', 'Configure an additional provider with embedding support: wunderland login', false);
    }
    /* ---- Model catalog -------------------------------------------- */
...
Each tool_call must include id (unique string), name (tool name), and a JSON body matching the tool's parameters schema.
    ...
    ensureInitialized() {
        if (!this.isInitialized) {
            throw new GeminiCLIProviderError('GeminiCLIProvider is not initialized. Call initialize() first.', 'UNKNOWN', 'Ensure the provider is initialized before making API calls.', false);
        }
    }
}
