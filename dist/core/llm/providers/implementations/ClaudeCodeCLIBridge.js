...
export class ClaudeCodeCLIBridge extends CLISubprocessBridge {
    ...
    classifyError(error) {
        ...
        if (error.timedOut || error.isTerminated) {
            return new ClaudeCodeProviderError(`Claude Code CLI timed out.`, 'TIMEOUT', 'Claude Code did not respond in time. Try again or use a smaller model (claude-haiku-4-5-20251001).', true, { exitCode, stderr });
        }
        /* Auth errors */
        if (stderr.includes('not logged in') || stderr.includes('authentication') || stderr.includes('unauthorized')) {
            return new ClaudeCodeProviderError('Claude Code is installed but not logged in.', 'NOT_AUTHENTICATED', 'Run "claude" in your terminal and complete the login flow, then try again.', false, { exitCode, stderr });
        }
        /* Rate limit */
        if (stderr.includes('rate limit') || stderr.includes('too many requests') || stderr.includes('429')) {
            return new ClaudeCodeProviderError('Claude Code subscription rate limit reached.', 'RATE_LIMITED', 'Wait a few minutes and try again. Check your Max plan limits at https://claude.ai/pricing', true, { exitCode, stderr });
        }
        /* Context too long */
        if (stderr.includes('context') && (stderr.includes('too long') || stderr.includes('token limit'))) {
            return new ClaudeCodeProviderError('Conversation exceeds model context window.', 'CONTEXT_TOO_LONG', 'Start a new conversation or switch to a model with a larger context window.', false, { exitCode, stderr });
        }
        /* Spawn failure (ENOENT = binary not found in this context) */
        if (error.code === 'ENOENT') {
            return new ClaudeCodeProviderError('Claude Code CLI not found.', 'BINARY_NOT_FOUND', 'Install Claude Code: npm install -g @anthropic-ai/claude-code — or download from https://claude.ai/download', false, { exitCode, stderr });
        }
        /* Permission error */
        if (error.code === 'EACCES') {
            return new ClaudeCodeProviderError('Failed to start Claude Code process due to permissions.', 'SPAWN_FAILED', `Check file permissions on the claude binary.`, false, { exitCode, stderr });
        }
        /* Generic crash */
        const stderrSnippet = stderr.length > 500 ? stderr.slice(-500) : stderr;
        return new ClaudeCodeProviderError(`Claude Code CLI exited unexpectedly (code ${exitCode ?? 'unknown'}).`, 'CRASHED', `Try running "claude --bare -p test" manually to diagnose. stderr: ${stderrSnippet}`, true, { exitCode, stderr });
    }
}
//# sourceMappingURL=ClaudeCodeCLIBridge.js.map