 ...
 */
import { ClaudeCodeCLIBridge } from './ClaudeCodeCLIBridge.js';
import { ClaudeCodeProviderError } from '../errors/ClaudeCodeProviderError.js';
/* ------------------------------------------------------------------ */
...
export class ClaudeCodeProvider {
    ...
    async initialize(config) {
        ...
        if (!installCheck.installed) {
            throw new ClaudeCodeProviderError('Claude Code CLI is not installed.', 'BINARY_NOT_FOUND', 'Install Claude Code: npm install -g @anthropic-ai/claude-code — or download from https://claude.ai/download\n\nThen log in by running "claude" in your terminal.\n\nAlternatively, switch to a different provider:\n  wunderland login', false);
        }
        /* 2. Check authentication */
        const isAuth = await this.bridge.checkAuthenticated();
        if (!isAuth) {
            throw new ClaudeCodeProviderError('Claude Code is installed but not logged in.', 'NOT_AUTHENTICATED', 'Open your terminal and run:\n  claude\n\nComplete the login flow with your Anthropic account, then restart your agent.', false);
        }
    ...
    async generateCompletion(modelId, messages, options) {
        ...
        if (result.isError) {
            throw new ClaudeCodeProviderError(`Claude Code returned an error: ${result.result}`, 'CRASHED', 'Try running "claude --bare -p test" manually to diagnose.', true);
        }
    ...
    async *generateCompletionStream(modelId, messages, options) {
        ...
        try {
                    ...
                    case 'result': {
                        ...
                        if (hasTools) {
                            try {
                                ...
                                if (parsed.response_type === 'tool_calls' && Array.isArray(parsed.tool_calls)) {
                                    emittedFinal = true;
                                    yield this.buildToolCallResponse(parsed.tool_calls, { sessionId: event.sessionId, usage: event.usage }, responseId, finalUsage, modelId);
                                    return;
                                }
                            }
                        ...
                        emittedFinal = true;
                        yield this.buildTextResponse(finalText, { sessionId: event.sessionId, usage: event.usage }, responseId, finalUsage, modelId);
                        return;
                    }
                    case 'error':
                        emittedFinal = true;
                        yield this.buildStreamErrorResponse(`Claude Code stream error: ${event.error}`, modelId, responseId, finalUsage);
                        return;
                }
            }
        }
            ...
            emittedFinal = true;
            yield this.buildStreamErrorResponse(error?.message ?? 'Claude Code stream failed.', modelId, responseId, finalUsage, error?.code, error);
            return;
        }
        if (!emittedFinal) {
    ...
    async generateEmbeddings(_modelId, _texts, _options) {
        throw new ClaudeCodeProviderError('Claude Code CLI does not support embeddings. Use a different provider (OpenAI, Ollama, etc.) for embedding operations.', 'EMBEDDINGS_NOT_SUPPORTED', 'Configure an additional provider with embedding support: wunderland login', false);
    }
    /* ---- Model catalog -------------------------------------------- */
     ...
     */
    serializeConversationXml(messages) {
        ...
        for (const msg of messages) {
            ...
            if (msg.role === 'tool') {
                ...
                const toolCallXml = msg.tool_calls.map(tc => `<tool_call name="${tc.function.name}">${tc.function.arguments}</tool_call>`).join('');
     ...
     */
    injectToolSchemas(systemPrompt, tools, toolChoice) {
...
Each tool call must include "id" (unique string), "name" (tool name), and "arguments" (object matching the tool's parameters schema).
    ...
    buildTextResponse(text, result, responseId, usage, modelId) {
    ...
    buildToolCallResponse(toolCalls, result, responseId, usage, modelId) {
        const u = usage ?? this.buildUsage(result.usage);
        return {
            id: responseId ?? `cc-${result.sessionId ?? Date.now()}`,
            object: 'chat.completion',
    ...
    buildStreamErrorResponse(message, modelId, responseId, usage, code, details) {
        return {
            id: responseId,
            object: 'chat.completion.chunk',
            created: Date.now(),
    ...
    ensureInitialized() {
        if (!this.isInitialized) {
            throw new ClaudeCodeProviderError('ClaudeCodeProvider is not initialized. Call initialize() first.', 'UNKNOWN', 'Ensure the provider is initialized before making API calls.', false);
        }
    }
}
