...
export class PromptEngine {
    ...
    async constructPrompt(baseComponents, modelTargetInfo, executionContext, templateName) {
        ...
        try {
            ...
            const augmentedComponents = this.augmentBaseComponents(baseComponents, selectedElements, executionContext);
    ...
    augmentBaseComponents(base, selectedElements, executionContext) {
        /**
    ...
    buildUserPreferencePrompts(executionContext) {
        ...
        if (verbosity === 'concise') {
            prompts.push({
                content: 'User response preference: keep the answer concise and efficient. Prefer brief explanations unless more detail is necessary.',
                priority: 90,
            ...
            prompts.push({
                content: 'User response preference: use a balanced level of detail. Be clear and complete without over-explaining.',
                priority: 90,
                source: 'user_preference:verbosity',
            });
        }
            ...
            prompts.push({
                content: 'User response preference: provide a detailed, thorough answer with enough explanation to justify the result.',
                priority: 90,
                source: 'user_preference:verbosity',
            });
        }
    ...
    normalizeVerbosityPreference(rawValue) {
    ...
    normalizePreferredFormatPreference(preferences) {
    ...
    buildUserPreferenceCacheKey(preferences) {
        if (!preferences || typeof preferences !== 'object') {
            return undefined;
        }
        const verbosity = this.normalizeVerbosityPreference(preferences.verbosity);
            ...
            if (components.conversationHistory) {
                components.conversationHistory.forEach(msg => {
                    ...
                    if (msg.role === MessageRole.ASSISTANT)
                        ...
                        role = 'tool';
                    else if (msg.role === MessageRole.SUMMARY)
                        role = 'system';
