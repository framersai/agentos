...
export class ConversationManager {
     ...
     */
    async initialize(config, utilityAIService, storageAdapter) {
        if (this.initialized) {
        ...
        console.log(`ConversationManager (ID: ${this.managerId}) initialized. Persistence: ${this.config.persistenceEnabled}. Append-only: ${this.config.appendOnlyPersistence}. Max in-memory: ${this.config.maxActiveConversationsInMemory}.`);
    }
    /**
     ...
     */
    async getOrCreateConversationContext(conversationId, userId, gmiInstanceId, activePersonaId, initialMetadata = {}, overrideConfig) {
        ...
        if (context) {
            ...
            if (userId && context.getMetadata('userId') !== userId)
                context.setMetadata('userId', userId);
            if (gmiInstanceId && context.getMetadata('gmiInstanceId') !== gmiInstanceId)
                context.setMetadata('gmiInstanceId', gmiInstanceId);
            if (activePersonaId && context.getMetadata('activePersonaId') !== activePersonaId)
                context.setMetadata('activePersonaId', activePersonaId);
        ...
        }; // Cast as fully required after merging
     ...
     */
    async listContextsForSession(sessionId) {
    ...
    async deleteConversation(conversationId) {
        ...
        if (this.config.appendOnlyPersistence) {
            throw new GMIError(`Conversation deletion is disabled when appendOnlyPersistence is enabled.`, GMIErrorCode.METHOD_NOT_SUPPORTED, { conversationId, managerId: this.managerId });
        }
    ...
    async getConversationInfo(sessionId) {
        ...
        if (this.config.persistenceEnabled && this.storageAdapter) {
            try {
                const dbConvo = await this.storageAdapter.get('SELECT id, created_at FROM conversations WHERE id = ?', [sessionId]);
                if (dbConvo) {
     ...
     */
    async getLastActiveTimeForConversation(conversationId) {
        this.ensureInitialized();
        ...
        if (this.config.persistenceEnabled && this.storageAdapter) {
            try {
                if (this.config.appendOnlyPersistence) {
                    const row = await this.storageAdapter.get('SELECT MAX(timestamp) AS last_ts FROM conversation_messages WHERE conversation_id = ?', [conversationId]);
                    if (row?.last_ts != null) {
                        return row.last_ts;
                    }
                    const convo = await this.storageAdapter.get('SELECT created_at FROM conversations WHERE id = ?', [conversationId]);
                    return convo?.created_at;
                }
                const convo = await this.storageAdapter.get('SELECT updated_at FROM conversations WHERE id = ?', [conversationId]);
     ...
     */
    async saveConversationToDB(context) {
        if (!this.storageAdapter || !this.config.persistenceEnabled)
            return;
        ...
        try {
            await this.storageAdapter.transaction(async (tx) => {
                ...
                const conversationData = {
                    ...
                    title: contextJSON.sessionMetadata?.title || `Conversation from ${new Date(context.createdAt).toLocaleDateString()}`,
                    language: context.currentLanguage || null,
                    session_details: JSON.stringify(contextJSON.sessionMetadata || {}),
                    is_archived: contextJSON.sessionMetadata?.isArchived ? 1 : 0,
                    tags: JSON.stringify(contextJSON.sessionMetadata?.tags || []),
                    created_at: context.createdAt,
                    updated_at: now,
                };
                // Upsert conversation (cross-platform compatible)
                // Check if exists first, then INSERT or UPDATE (unless append-only).
                const existing = await tx.get('SELECT created_at FROM conversations WHERE id = ?', [conversationData.id]);
                const finalCreatedAt = existing?.created_at || conversationData.created_at;
                if (!existing) {
                    await tx.run(`INSERT INTO conversations (id, user_id, gmi_instance_id, title, language, session_details, is_archived, tags, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                        conversationData.id,
                    ...
                    await tx.run(`UPDATE conversations SET
             ...
             WHERE id = ?`, [
                ...
                if (contextJSON.messages && contextJSON.messages.length > 0) {
                    for (const msg of contextJSON.messages) {
                        const messageId = msg.id || `msg_${uuidv4()}`;
                        // Append-only persistence: insert each message once; never update.
                        if (this.config.appendOnlyPersistence) {
                            const existingMsg = await tx.get('SELECT id FROM conversation_messages WHERE id = ?', [messageId]);
                            if (existingMsg) {
                                continue;
                            }
                            await tx.run(`INSERT INTO conversation_messages (
                  ...
                  multimodal_data, audio_url, audio_transcript, voice_settings, metadata
                                ...
                                msg.multimodalData ? JSON.stringify(msg.multimodalData) : null,
                                msg.audioUrl || null,
                                msg.audioTranscript || null,
                                msg.voiceSettings ? JSON.stringify(msg.voiceSettings) : null,
                        ...
                        if (tx.kind !== 'postgres') {
                            await tx.run(`INSERT OR REPLACE INTO conversation_messages (
                  id, conversation_id, role, content, timestamp, tool_calls, tool_call_id,
                  multimodal_data, audio_url, audio_transcript, voice_settings, metadata
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                                messageId,
                            ...
                            const existingPgMsg = await tx.get('SELECT id FROM conversation_messages WHERE id = ?', [messageId]);
                            if (!existingPgMsg) {
                                await tx.run(`INSERT INTO conversation_messages (
                    id, conversation_id, role, content, timestamp, tool_calls, tool_call_id,
                    multimodal_data, audio_url, audio_transcript, voice_settings, metadata
                                ...
                                await tx.run(`UPDATE conversation_messages SET
    ...
    async loadConversationFromDB(conversationId) {
        if (!this.storageAdapter || !this.config.persistenceEnabled)
            return undefined;
        try {
            const convoData = await this.storageAdapter.get('SELECT * FROM conversations WHERE id = ?', [conversationId]);
            if (!convoData)
                return undefined;
            const messages = await this.storageAdapter.all('SELECT * FROM conversation_messages WHERE conversation_id = ? ORDER BY timestamp ASC', [conversationId]);
            const internalMessages = messages.map((dbMsg) => {
                let parsedContent;
                try {
                ...
                return createConversationMessage(dbMsg.role, parsedContent, {
                    ...messageOptions,
                    id: dbMsg.id,
                    timestamp: dbMsg.timestamp,
                });
            });
            const sessionMetadataFromDB = JSON.parse(convoData.session_details || '{}');
            const contextSpecificConfig = sessionMetadataFromDB.contextConfigOverrides || {};
            const finalContextConfig = {
                ...this.config.defaultConversationContextConfig,
                ...
                activePersonaId: sessionMetadataFromDB.activePersonaId || this.config.defaultConversationContextConfig?.activePersonaId,
