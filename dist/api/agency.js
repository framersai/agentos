...
export function agency(opts) {
    ...
    const hasDependsOn = Object.values(resolvedAgents).some((a) => !isAgent(a) && Array.isArray(a.dependsOn) && a.dependsOn.length > 0);
    ...
    const prepareExecutionPrompt = async (prompt) => {
    ...
    const finalizeExecutionResult = async (result, start, sessionId, streamPartBuffer) => {
        const guardConfig = normalizeGuardrails(opts.guardrails);
        ...
        const approvedResult = await maybeApproveFinalResult(opts, agencyName, finalized, elapsedMs, streamPartBuffer
            ? (part) => {
        ...
        streamPartBuffer?.push({
            type: 'final-output',
            text: approvedResult.text ?? '',
            usage: normalizeUsage(approvedResult.usage),
            agentCalls: (approvedResult.agentCalls ?? []),
            parsed: approvedResult.parsed,
            durationMs: elapsedMs,
        });
        opts.on?.agentEnd?.({
            agent: agencyName,
            output: approvedResult.text ?? '',
            durationMs: elapsedMs,
            timestamp: Date.now(),
        });
    ...
    const wrappedExecute = async (prompt, execOpts, sessionId) => {
        ...
        try {
            ...
            const result = (await strategy.execute(preparedPrompt, execOpts));
    ...
    const createStreamResult = (prompt, streamOpts, sessionId) => {
        const start = Date.now();
        let errorReported = false;
        const postStreamParts = [];
        ...
        const deferredStream = (async () => {
            const preparedPrompt = await prepareExecutionPrompt(prompt);
            return strategy.stream(preparedPrompt, streamOpts);
        })();
        const rawPartReplay = createBufferedAsyncReplay((async function* () {
        ...
        const resolvedTextPromise = (async () => {
        ...
        const resolvedUsagePromise = (async () => {
            const streamResult = await deferredStream;
        ...
        const resolvedAgentCallsPromise = (async () => {
            const streamResult = await deferredStream;
        ...
        const finalizedResultPromise = (async () => {
            try {
        ...
        return {
            ...
            fullStream: (async function* () {
                try {
                    ...
                    const hasMatchingAgencyEnd = allBufferedParts.some((part) => part.type === 'agent-end' &&
                        part.agent === agencyName &&
                        part.output === (finalResult.text ?? ''));
            ...
            text: finalizedResultPromise.then((result) => result.text ?? ''),
            usage: finalizedResultPromise.then((result) => result.usage),
            agentCalls: finalizedResultPromise.then((result) => (result.agentCalls ?? [])),
            ...
            if (!sessions.has(sessionId)) {
                ...
                const sessionObj = {
                    ...
                    stream(text) {
                        ...
                        const streamResult = createStreamResult(fullPrompt, undefined, sessionId);
    ...
    for (const [name, agentOrConfig] of Object.entries(opts.agents)) {
        if (isAgent(agentOrConfig)) {
            ...
            agencySubAgentConfigs[name] = agentOrConfig;
        }
    }
                ...
                try {
                    ...
                    if (adapterModule?.createExtensionPack) {
                        ...
                        if (adapter && typeof adapter.connect === 'function') {
                            await adapter.connect(channelConfig, async (msg) => {
...
async function runGuardrails(text, guardIds, direction, callbacks) {
    if (!guardIds.length)
        return text;
    try {
        const { ParallelGuardrailDispatcher, GuardrailAction } = await import('../safety/guardrails/index.js');
        /*
...
async function maybeApproveFinalResult(opts, agencyName, result, elapsedMs, emitStreamPart) {
    ...
    if (decision.approved && opts.hitl.guardrailOverride !== false) {
        ...
        const overrideResult = await runPostApprovalGuardrails('return', { output: result.text ?? '' }, postGuardrails, opts.on);
        if (!overrideResult.passed) {
            ...
            throw new AgencyConfigError(`Guardrail overrode HITL approval for final output — ${overrideResult.guardrailId}: ${overrideResult.reason}`);
        }
    }
 ...
 */
export async function runPostApprovalGuardrails(toolName, args, guardrailIds, callbacks) {
    ...
    for (const guardId of guardrailIds) {
        try {
            ...
            if (result.action === 'block') {
                ...
                console.warn(`[Guardrail] Overrode HITL approval for tool "${toolName}" — ${guardId}: ${reason}`);
                callbacks?.guardrailResult?.({
                    agent: '__agency__',
                    guardrailId: guardId,
                    passed: false,
             ...
             */
            console.warn(`[Guardrail] Post-approval guardrail "${guardId}" threw for tool "${toolName}" — skipping`);
        }
    }
    return { passed: true };
}
 ...
 */
function evaluatePostApprovalGuardrail(guardId, payload) {
    switch (guardId) {
        case 'code-safety': {
            /*
