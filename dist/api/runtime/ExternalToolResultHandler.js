...
export class ExternalToolResultHandler {
     ...
     */
    constructor(activeStreamContexts, chunks, dependencies, enableConversationalPersistence, processGMIOutputCallback, resolveOrganizationContext) {
        this.activeStreamContexts = activeStreamContexts;
        this.chunks = chunks;
        this.dependencies = dependencies;
        this.enableConversationalPersistence = enableConversationalPersistence;
        this.processGMIOutputCallback = processGMIOutputCallback;
        this.resolveOrganizationContext = resolveOrganizationContext;
    }
    /**
     ...
     */
    setProcessGMIOutputCallback(cb) {
    ...
    async orchestrateToolResults(agentOSStreamId, toolResults) {
        ...
        try {
            await withAgentOSSpan('agentos.tool_result', async (span) => {
                ...
                span?.setAttribute('agentos.tool_call_ids', JSON.stringify(toolResults.map((tr) => tr.toolCallId)));
                span?.setAttribute('agentos.tool_names', JSON.stringify(toolResults.map((tr) => tr.toolName)));
                ...
                try {
                    ...
                    const gmiOutputAfterTool = await withAgentOSSpan('agentos.gmi.handle_tool_result', async (child) => {
                        child?.setAttribute('agentos.stream_id', agentOSStreamId);
                        child?.setAttribute('agentos.tool_result_count', toolResults.length);
                        child?.setAttribute('agentos.tool_call_ids', JSON.stringify(toolResults.map((tr) => tr.toolCallId)));
                        child?.setAttribute('agentos.tool_names', JSON.stringify(toolResults.map((tr) => tr.toolName)));
                        ...
                        if (toolResults.length > 1) {
                            ...
                            return gmi.handleToolResults(toolResults.map((tr) => ({
                                toolCallId: tr.toolCallId,
                                toolName: tr.toolName,
                                output: tr.isSuccess
                                    ? tr.toolOutput
     ...
     */
    async resumeToolResultsInternal(agentOSStreamId, pendingRequest, toolResults, options) {
        if (!Array.isArray(toolResults) || toolResults.length === 0) {
...
//# sourceMappingURL=ExternalToolResultHandler.js.map