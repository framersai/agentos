...
import { GMIOutputChunkType, } from '../../cognitive_substrate/IGMI.js';
...
import { recordAgentOSTurnMetrics, recordExceptionOnActiveSpan, runWithSpanContext, startAgentOSSpan, withAgentOSSpan, } from '../../evaluation/observability/otel.js';
...
import { ExternalToolResultHandler } from './ExternalToolResultHandler.js';
import { GMIChunkTransformer } from './GMIChunkTransformer.js';
...
export class AgentOSOrchestrator {
    ...
    async initialize(config, dependencies) {
        ...
        this.chunkTransformer = new GMIChunkTransformer(this.activeStreamContexts, this.chunks, this.dependencies, this.config.enableConversationalPersistence, 
        // clearPendingRequest callback — wired after toolResultHandler is created below
        async () => { });
        this.toolResultHandler = new ExternalToolResultHandler(this.activeStreamContexts, this.chunks, this.dependencies, this.config.enableConversationalPersistence, this.chunkTransformer.processGMIOutput.bind(this.chunkTransformer), this.resolveOrganizationContext.bind(this));
        // Wire the chunkTransformer's clearPendingRequest to the toolResultHandler
        this.chunkTransformer.setClearPendingRequestCallback(this.toolResultHandler.clearPendingExternalToolRequest.bind(this.toolResultHandler));
        this.turnPipeline = new TurnExecutionPipeline(this.activeStreamContexts, this.chunks, this.dependencies, this.config, this.chunkTransformer, this.telemetry, this.resolveOrganizationContext.bind(this));
    ...
    async orchestrateResumedToolResults(pendingRequest, toolResults, options = {}) {
        ...
        const run = async () => this.toolResultHandler.resumeToolResultsInternal(agentOSStreamId, pendingRequest, toolResults, options);
        const promise = rootSpan ? runWithSpanContext(rootSpan, run) : run();
        promise
            .catch(async (criticalError) => {
            if (rootSpan) {
        ...
        try {
            ...
            while (continueProcessing && currentToolCallIteration < this.config.maxToolCallIterations) {
                ...
                if (lastGMIOutput?.toolCalls && lastGMIOutput.toolCalls.length > 0) {
                    ...
                    await this.chunkTransformer.processGMIOutput(agentOSStreamId, streamContext, lastGMIOutput, true /*isContinuation*/);
                    if (lastGMIOutput.toolCalls && lastGMIOutput.toolCalls.length > 0) {
            ...
            if (finalGMIStateForResponse.isFinal === false &&
                ...
                if (conversationContext) {
                    await this.toolResultHandler.persistPendingExternalToolRequest(agentOSStreamId, streamContext, gmiInstanceIdForChunks, finalGMIStateForResponse.toolCalls, finalGMIStateForResponse.responseText ||
            ...
            if (typeof finalGMIStateForResponse.responseText === 'string' &&
                ...
                try {
                    ...
                    console.warn(`AgentOSOrchestrator: Failed to record long-term memory feedback for stream ${agentOSStreamId}.`, feedbackError);
                }
            }
            ...
            if (!keepStreamContextActive) {
                ...
                console.log(`AgentOSOrchestrator: Stream ${agentOSStreamId} retained for external tool continuation.`);
            }
        }
    }
    /**
    ...
    async orchestrateToolResult(agentOSStreamId, toolCallId, toolName, toolOutput, isSuccess, errorMessage) {
        this.ensureInitialized();
        return this.toolResultHandler.orchestrateToolResult(agentOSStreamId, toolCallId, toolName, toolOutput, isSuccess, errorMessage);
    }
    /**
