 ...
 */
export function accumulateCacheTokens(totalUsage, call) {
    if (!call)
        return;
...
export async function checkBeforeAgent(name, context, agentCalls, agencyConfig) {
    ...
    if (decision.approved && agencyConfig.hitl?.guardrailOverride !== false) {
        ...
        const overrideResult = await runPostApprovalGuardrails(`agent:${name}`, { input: context }, postGuardrails, agencyConfig.on);
