...
export class AgentGraphBuilder {
    ...
    compile() {
        ...
        for (const [name, reg] of this.agents) {
            for (const dep of reg.config.dependsOn ?? []) {
                if (!this.agents.has(dep)) {
                    throw new Error(`Agent "${name}" depends on "${dep}" which is not registered in the graph`);
                }
            }
        }
        // ------------------------------------------------------------------
        ...
        if (sorted.length < this.agents.size) {
            ...
            throw new Error(`Cycle detected in agent graph. Stuck agents: ${stuck.join(', ')}`);
        }
        // ------------------------------------------------------------------
        ...
        for (const [name, reg] of this.agents) {
            ...
            nodes.push({
                ...
                executorConfig: {
                    type: 'gmi',
                    instructions: `You are agent "${name}". ${config.instructions}${contextInstructions}` +
...
//# sourceMappingURL=agentGraphBuilder.js.map