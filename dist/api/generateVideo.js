...
function emitVideoProgress(callback, status, progress, message) {
    if (!callback)
        return;
    try {
 ...
 */
function autoDetectVideoProvider() {
 ...
 */
function detectAvailableVideoProviders() {
    const available = [];
    for (const { envKey, providerId } of VIDEO_PROVIDER_ENV_MAP) {
        if (!process.env[envKey])
            continue;
        if (!hasVideoProviderFactory(providerId))
            continue;
 ...
 */
function detectFallbackVideoProviders(primaryProviderId) {
    const fallbacks = [];
    for (const { envKey, providerId } of VIDEO_PROVIDER_ENV_MAP) {
 ...
 */
async function createVideoProviderWithFallback(providerChain, apiKey, modelId, baseUrl, timeoutMs) {
...
export async function generateVideo(opts) {
    ...
    try {
        return await withAgentOSSpan('agentos.api.generate_video', async (span) => {
            ...
            if (opts.provider) {
                ...
                apiKey =
                    opts.apiKey ??
                        ...
                        '';
                if (!apiKey) {
                    throw new Error(`No API key for video provider "${providerId}". Set ${envKeyForProvider(providerId)} or pass apiKey.`);
                }
                ...
                providerChain = resolveProviderChain(detectAvailableVideoProviders(), opts.providerPreferences);
                if (providerChain.length === 0) {
                    throw new Error('No video provider configured. Set RUNWAY_API_KEY, REPLICATE_API_TOKEN, or FAL_API_KEY.');
                }
            ...
            const provider = await createVideoProviderWithFallback(providerChain, apiKey, opts.model, opts.baseUrl, opts.timeoutMs);
            ...
            if (opts.image) {
                ...
                result = await provider.imageToVideo({
                    modelId: provider instanceof FallbackVideoProxy
                        ? undefined
        ...
        metricStatus = 'error';
        emitVideoProgress(opts.onProgress, 'failed', 100, error instanceof Error ? error.message : String(error));
        throw error;
    }
    finally {
        try {
