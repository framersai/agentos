...
function matchesFilter(metadata, filter) {
    for (const [key, condition] of Object.entries(filter)) {
        ...
        if (cond.$eq !== undefined && value !== cond.$eq)
            return false;
        if (cond.$ne !== undefined && value === cond.$ne)
            return false;
        if (cond.$gt !== undefined && (typeof value !== 'number' || value <= cond.$gt))
            return false;
        if (cond.$gte !== undefined && (typeof value !== 'number' || value < cond.$gte))
            return false;
        if (cond.$lt !== undefined && (typeof value !== 'number' || value >= cond.$lt))
            return false;
        if (cond.$lte !== undefined && (typeof value !== 'number' || value > cond.$lte))
            return false;
        ...
        if (cond.$contains !== undefined) {
            if (Array.isArray(value)) {
                if (!value.includes(cond.$contains))
                    return false;
            }
            else if (typeof value === 'string') {
                if (!value.includes(String(cond.$contains)))
                    return false;
            }
            else {
                return false;
        ...
        if (cond.$all !== undefined) {
            if (!Array.isArray(value))
                return false;
            if (!cond.$all.every((v) => value.includes(v)))
                return false;
        }
        if (cond.$textSearch !== undefined) {
            const text = typeof value === 'string' ? value : JSON.stringify(value);
            if (!text.toLowerCase().includes(cond.$textSearch.toLowerCase()))
                return false;
        }
    }
    return true;
...
export class Neo4jVectorStore {
    ...
    async initialize(config) {
        ...
        await this.cypher.writeVoid(`CREATE CONSTRAINT vec_doc_unique IF NOT EXISTS FOR (n:${VECTOR_DOC_LABEL}) REQUIRE (n.docId, n.collectionName) IS UNIQUE`);
    }
    async createCollection(collectionName, dimension, options) {
        ...
        // Create vector index
        await this.cypher.writeVoid(`CREATE VECTOR INDEX ${idxName} IF NOT EXISTS
       ...
       OPTIONS {
        ...
        await this.cypher.writeVoid(`CREATE FULLTEXT INDEX ${ftName} IF NOT EXISTS
       FOR (n:${VECTOR_DOC_LABEL})
    ...
    async upsert(collectionName, documents, options) {
        ...
        for (let i = 0; i < documents.length; i += batchSize) {
            ...
            try {
                const results = await this.cypher.write(`UNWIND $docs AS doc
           ...
           RETURN doc.id AS id`, { docs, collectionName });
    ...
    async query(collectionName, queryEmbedding, options) {
        ...
        const results = await this.cypher.read(`CALL db.index.vector.queryNodes($idxName, $fetchK, $queryEmbedding)
       YIELD node, score
       ...
       RETURN node.docId AS docId,
              ...
              score
       ORDER BY score DESC`, { idxName, fetchK, queryEmbedding, collectionName });
        let documents = results.map((r) => {
            const doc = {
                id: r.docId,
        ...
        if (hasFilter && options?.filter) {
            ...
            documents = documents.filter((d) => {
                const meta = d.metadata ?? deserializeMetadata(results.find((r) => r.docId === d.id)?.metadata_json);
    ...
    async hybridSearch(collectionName, queryEmbedding, queryText, options) {
        ...
        const ftName = fulltextIndexName(this.indexPrefix, collectionName);
        const lexResults = await this.cypher.read(`CALL db.index.fulltext.queryNodes($ftName, $queryText)
       YIELD node, score
       WHERE node.collectionName = $collectionName
       RETURN node.docId AS docId,
              node.embedding AS embedding,
              ...
              node.metadata_json AS metadata_json,
              score
       LIMIT $lexicalTopK`, { ftName, queryText, collectionName, lexicalTopK });
        // RRF fusion
        ...
        const documents = scored.slice(0, topK).map((s) => {
    ...
    async delete(collectionName, ids, options) {
        if (options?.deleteAll) {
            const results = await this.cypher.write(`MATCH (n:${VECTOR_DOC_LABEL} { collectionName: $collectionName })
         WITH n, count(n) AS total
         DETACH DELETE n
         RETURN total AS count`, { collectionName });
        ...
        if (ids && ids.length > 0) {
         ...
         DETACH DELETE n
         RETURN cnt AS deletedCount`, { collectionName, ids });
    ...
    async deleteCollection(collectionName) {
        ...
        await this.cypher.writeVoid(`MATCH (n:${VECTOR_DOC_LABEL} { collectionName: $collectionName }) DETACH DELETE n`, { collectionName });
    }
    async collectionExists(collectionName) {
        const idxName = vectorIndexName(this.indexPrefix, collectionName);
        const results = await this.cypher.read(`SHOW INDEXES
       YIELD name, type
       WHERE name = $idxName AND type = 'VECTOR'
       RETURN count(*) > 0 AS exists`, { idxName });
    ...
    async getStats(collectionName) {
        if (collectionName) {
            const results = await this.cypher.read(`MATCH (n:${VECTOR_DOC_LABEL} { collectionName: $collectionName })
         RETURN count(n) AS count`, { collectionName });
            return { collectionName, documentCount: Number(results[0]?.count ?? 0) };
        }
        const results = await this.cypher.read(`MATCH (n:${VECTOR_DOC_LABEL})
...
//# sourceMappingURL=Neo4jVectorStore.js.map