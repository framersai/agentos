...
export class InMemoryVectorStore {
    ...
    async initialize(config) {
        if (this.isInitialized) {
            console.warn(`InMemoryVectorStore (ID: ${this.providerId}, Config ID: ${config.id}) already initialized. Re-initializing.`);
            // Potentially clear state or handle re-initialization logic
            this.collections.clear();
        }
        if (config.type !== 'in_memory') {
            throw new GMIError(`Invalid configuration type for InMemoryVectorStore: ${config.type}. Expected 'in_memory'.`, 'CONFIG_ERROR');
        }
        this.config = config;
        if (this.config.persistPath) {
            try {
                await this.loadFromFile(this.config.persistPath);
                console.log(`InMemoryVectorStore (ID: ${this.providerId}) loaded data from ${this.config.persistPath}`);
            }
            catch (error) {
                if (error.code === 'ENOENT') {
                    console.log(`InMemoryVectorStore (ID: ${this.providerId}): Persistence file ${this.config.persistPath} not found. Starting with an empty store.`);
                }
                else {
                    console.warn(`InMemoryVectorStore (ID: ${this.providerId}): Failed to load data from ${this.config.persistPath}. Error: ${error.message}. Starting with an empty store.`);
                }
            }
        }
        this.isInitialized = true;
        console.log(`InMemoryVectorStore (ID: ${this.providerId}, Config ID: ${this.config.id}) initialized successfully.`);
    }
    /**
    ...
    ensureInitialized() {
        if (!this.isInitialized) {
            throw new GMIError(`InMemoryVectorStore (ID: ${this.providerId}) is not initialized. Call initialize() first.`, 'NOT_INITIALIZED');
        }
    }
    /**
    ...
    getCollection(collectionName) {
        ...
        if (!collection) {
            throw new GMIError(`Collection '${collectionName}' not found in InMemoryVectorStore (ID: ${this.providerId}).`, 'NOT_FOUND', { collectionName });
        }
        return collection;
    }
    /**
    ...
    async createCollection(collectionName, dimension, options) {
        ...
        if (this.collections.has(collectionName)) {
            if (options?.overwriteIfExists) {
                console.warn(`InMemoryVectorStore (ID: ${this.providerId}): Collection '${collectionName}' already exists and will be overwritten.`);
            }
            else {
                throw new GMIError(`Collection '${collectionName}' already exists in InMemoryVectorStore (ID: ${this.providerId}).`, 'ALREADY_EXISTS', { collectionName });
            }
        }
        if (dimension <= 0) {
            throw new GMIError(`Invalid dimension for collection '${collectionName}': ${dimension}. Must be positive.`, 'VALIDATION_ERROR', { dimension });
        }
        ...
        console.log(`InMemoryVectorStore (ID: ${this.providerId}): Collection '${collectionName}' created with dimension ${dimension}.`);
    }
    /**
     * @inheritdoc
     */
    ...
    async deleteCollection(collectionName) {
        this.ensureInitialized();
        if (!this.collections.has(collectionName)) {
            console.warn(`InMemoryVectorStore (ID: ${this.providerId}): Attempted to delete non-existent collection '${collectionName}'.`);
            return; // Or throw NOT_FOUND error, for now, be lenient.
        }
        this.collections.delete(collectionName);
        console.log(`InMemoryVectorStore (ID: ${this.providerId}): Collection '${collectionName}' deleted.`);
    }
    /**
     * @inheritdoc
     */
    ...
    async query(collectionName, queryEmbedding, options) {
        ...
        if (queryEmbedding.length !== collection.dimension) {
            throw new GMIError(`Query embedding dimension ${queryEmbedding.length} does not match collection '${collectionName}' dimension ${collection.dimension}.`, 'VALIDATION_ERROR');
        }
        const candidates = [];
        for (const doc of collection.documents.values()) {
            ...
            if (options?.includeEmbedding)
                retrievedDoc.embedding = doc.embedding;
            if (options?.includeMetadata !== false && doc.metadata)
                retrievedDoc.metadata = doc.metadata; // Default true
            if (options?.includeTextContent && doc.textContent)
                retrievedDoc.textContent = doc.textContent;
    ...
    async checkHealth() {
        ...
        return {
            ...
            details: {
                ...
                totalDocuments: Array.from(this.collections.values()).reduce((sum, col) => sum + col.documents.size, 0),
    ...
    async shutdown() {
        ...
        if (this.config.persistPath) {
            try {
                ...
                console.log(`InMemoryVectorStore (ID: ${this.providerId}) data persisted to ${this.config.persistPath} on shutdown.`);
            }
            catch (error) {
                console.error(`InMemoryVectorStore (ID: ${this.providerId}): Failed to persist data to ${this.config.persistPath} on shutdown. Error: ${error.message}`);
            }
        }
        this.collections.clear();
    ...
    matchesFilter(doc, filter) {
        if (!doc.metadata) {
            ...
            for (const key in filter) {
                ...
                if (typeof filterValue === 'object' && filterValue !== null && filterValue.$exists === true) {
                    return false; // Requires existence, but metadata is undefined
                }
                if (typeof filterValue !== 'object' || filterValue.$exists !== false) {
        ...
        for (const key in filter) {
            ...
            if (typeof filterValueOrCondition === 'object' && filterValueOrCondition !== null) {
                ...
                const condition = filterValueOrCondition;
                ...
                const filterScalarValue = filterValueOrCondition;
                if (Array.isArray(docValue)) {
                    if (!docValue.includes(filterScalarValue))
                        return false; // Check if scalar is in doc array
                }
    ...
    evaluateCondition(docValue, condition) {
        ...
        if (docValue === undefined)
            return false; // Field doesn't exist but condition isn't about existence
        ...
        if (Array.isArray(docValue)) {
            if (condition.$contains !== undefined) {
                if (!docValue.includes(condition.$contains))
                    return false;
            }
            if (condition.$all !== undefined) {
                if (!Array.isArray(condition.$all) || !condition.$all.every(item => docValue.includes(item)))
                    return false;
            }
        }
        ...
        if (data.collections) {
            ...
            for (const name in data.collections) {
                ...
                this.collections.set(name, {
                    ...
                    documents: new Map(colData.documents),
                });
            }
        }
    }
