 ...
 */
import { InMemoryVectorStore } from './vector_stores/InMemoryVectorStore.js';
import { SqlVectorStore } from './vector_stores/SqlVectorStore.js';
import { HnswlibVectorStore } from './vector_stores/HnswlibVectorStore.js';
import { QdrantVectorStore } from './vector_stores/QdrantVectorStore.js';
...
export class VectorStoreManager {
     ...
     * @inheritdoc
     */
    async initialize(managerConfig, dataSourceConfigs) {
    ...
    ensureInitialized() {
        if (!this.isInitialized) {
            throw new GMIError(`VectorStoreManager (ID: ${this.managerInstanceId}) is not initialized. Call initialize() first.`, GMIErrorCode.NOT_INITIALIZED);
        }
    }
    /**
    ...
    createProviderInstance(providerConfig) {
        switch (providerConfig.type) {
            ...
            case 'hnswlib':
                ...
                console.warn('[DEPRECATED] VectorStore type "hnswlib" is deprecated. Use type "sql" instead — ' +
            ...
            default:
                throw new GMIError(`Unsupported vector store provider type: '${providerConfig.type}' for provider ID '${providerConfig.id}'.`, GMIErrorCode.NOT_SUPPORTED, { providerType: providerConfig.type, providerId: providerConfig.id });
        }
    }
    /**
     * @inheritdoc
    ...
    getProvider(providerId) {
        ...
        if (!provider) {
            throw new GMIError(`Vector store provider with ID '${providerId}' not found or not initialized in VectorStoreManager (ID: ${this.managerInstanceId}).`, GMIErrorCode.NOT_FOUND, { providerId });
        }
        return provider;
    }
    /**
    ...
    getDefaultProvider() {
        ...
        if (!defaultProviderId) {
            ...
            throw new GMIError(`No default vector store provider ID configured in VectorStoreManager (ID: ${this.managerInstanceId}).`, GMIErrorCode.CONFIG_ERROR);
        }
     ...
     * @inheritdoc
     */
    async getStoreForDataSource(dataSourceId) {
        this.ensureInitialized();
        const mappingInfo = this.dataSourceMappings.get(dataSourceId);
        if (!mappingInfo) {
            throw new GMIError(`Data source mapping for ID '${dataSourceId}' not found in VectorStoreManager (ID: ${this.managerInstanceId}).`, GMIErrorCode.NOT_FOUND, { dataSourceId });
        }
    ...
    async checkHealth(providerId) {
        ...
        if (providerId && providersToCheck.length === 1 && report.providerStatus && report.providerStatus[providerId]) {
            ...
            if (this.managerConfig.providers?.length === 0) {
                report.isOverallHealthy = false;
                if (report.managerDetails)
                    report.managerDetails.statusMessage = "No providers configured.";
            }
        }
        return report;
    }
