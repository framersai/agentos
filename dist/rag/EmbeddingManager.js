...
export class EmbeddingManager {
     ...
     * @inheritdoc
     */
    async initialize(config, providerManager) {
        if (this.initialized) {
        ...
        if (!config.embeddingModels || config.embeddingModels.length === 0) {
            throw new GMIError('EmbeddingManagerConfig: No embedding models provided. At least one model must be configured.', GMIErrorCode.CONFIG_ERROR, { modelsCount: 0 });
        }
        config.embeddingModels.forEach(modelConfig => {
            if (!modelConfig.modelId || !modelConfig.providerId || !modelConfig.dimension) {
                console.warn(`EmbeddingManager: Invalid or incomplete model configuration skipped: ${JSON.stringify(modelConfig)}. ModelId, providerId, and dimension are required.`);
                return;
            }
            if (modelConfig.dimension <= 0) {
                console.warn(`EmbeddingManager: Model configuration for '${modelConfig.modelId}' has an invalid dimension (${modelConfig.dimension}). Skipping.`);
                return;
            }
        ...
        if (config.defaultModelId) {
            ...
            if (!modelFromId) {
                throw new GMIError(`EmbeddingManagerConfig: Specified defaultModelId '${config.defaultModelId}' does not match any configured embedding model.`, GMIErrorCode.CONFIG_ERROR, { defaultModelId: config.defaultModelId });
            }
        ...
        if (!this.defaultModel && this.availableModels.size > 0) {
            ...
            console.warn(`EmbeddingManager: No default embedding model explicitly set via 'isDefault' or 'defaultModelId'. Using the first available model as default: '${this.defaultModel?.modelId}'.`);
        }
        if (!this.defaultModel) {
            throw new GMIError('EmbeddingManager: No default embedding model could be determined. Ensure at least one model is configured, and optionally specify a default.', GMIErrorCode.CONFIG_ERROR);
        }
        if (this.config.enableCache !== false) { // Cache is enabled by default
            ...
            console.debug(`EmbeddingManager: Cache enabled (maxSize: ${cacheMaxSize}, ttl: ${cacheTTLSeconds}s).`);
        }
        else {
            console.debug('EmbeddingManager: Cache is disabled by configuration.');
        }
        this.initialized = true;
        console.debug(`EmbeddingManager initialized successfully with ${this.availableModels.size} embedding model(s). Default model: '${this.defaultModel?.modelId}'.`);
    }
    /**
    ...
    ensureInitialized() {
        if (!this.initialized) {
            throw new GMIError('EmbeddingManager is not initialized. Call initialize() before using its methods.', GMIErrorCode.NOT_INITIALIZED);
        }
    }
    /**
    ...
    selectModel(request) {
        ...
        if (request.modelId) {
            ...
            if (model) {
                ...
                if (request.providerId && request.providerId !== model.providerId) {
                    console.warn(`EmbeddingManager: Requested providerId '${request.providerId}' for model '${request.modelId}' differs from the model's configured provider '${model.providerId}'. Using the model's configured provider.`);
                }
                return model;
            }
            console.warn(`EmbeddingManager: Requested modelId '${request.modelId}' not found among available models. Falling back to selection strategy or default.`);
        }
        ...
        if (strategyConfig) {
            switch (strategyConfig.type) {
                ...
                case 'dynamic_collection_preference':
                    if (request.collectionId) {
                        selectedModel = [...this.availableModels.values()].find(m => m.supportedCollections?.includes(request.collectionId));
                    }
                    break;
            ...
            if (strategyConfig.fallbackModelId) {
                ...
                if (selectedModel)
                    return selectedModel;
                console.warn(`EmbeddingManager: Selection strategy fallbackModelId '${strategyConfig.fallbackModelId}' not found. Using system default.`);
            }
        }
    ...
    async generateEmbeddings(request) {
        ...
        if (!provider) {
            throw new GMIError(`LLM Provider '${selectedModelConfig.providerId}' configured for embedding model '${selectedModelConfig.modelId}' not found or not available.`, GMIErrorCode.PROVIDER_NOT_FOUND, { providerId: selectedModelConfig.providerId, modelId: selectedModelConfig.modelId });
        }
        if (typeof provider.generateEmbeddings !== 'function') {
            throw new GMIError(`LLM Provider '${selectedModelConfig.providerId}' does not support the 'generateEmbeddings' method required by model '${selectedModelConfig.modelId}'.`, GMIErrorCode.METHOD_NOT_SUPPORTED, { providerId: selectedModelConfig.providerId, method: 'generateEmbeddings' });
        }
        const finalEmbeddings = new Array(textsToEmbed.length).fill(null).map(() => []); // Initialize to avoid sparse arrays if errors occur
        const errors = [];
        ...
        for (let i = 0; i < textsToEmbed.length; i += batchSize) {
            ...
            const textsToFetchFromProvider = [];
            // Map: index_in_textsToFetchFromProvider -> original_overall_index
            const providerFetchIndexToOriginalIndexMap = [];
            ...
            if (textsToFetchFromProvider.length > 0) {
                try {
                    const providerOptions = {
                        ...
                        dimensions: selectedModelConfig.providerSpecificArgs?.dimensions,
                        // inputType for older OpenAI models if still supported by provider interface
                        inputType: selectedModelConfig.providerSpecificArgs?.inputType,
                    };
                    const batchResponse = await provider.generateEmbeddings(selectedModelConfig.modelId, textsToFetchFromProvider, providerOptions);
                    ...
                    if (batchResponse.data && batchResponse.data.length === textsToFetchFromProvider.length) {
                        ...
                        throw new GMIError('Provider returned mismatched number of embeddings for the batch or no data.', GMIErrorCode.PROVIDER_ERROR, { expected: textsToFetchFromProvider.length, received: batchResponse.data?.length });
                    }
                    ...
                    if (isNetworkError) {
                        ...
                        console.debug(`[Embeddings] Error embedding batch with model '${selectedModelConfig.modelId}' via provider '${provider.providerId}': ${errorMessage}`);
                    }
        ...
        const successfullyProcessedEmbeddings = [];
        const finalOriginalIndices = []; // To map errors back correctly if we filter embeddings
     ...
     * @inheritdoc
     */
    async getEmbeddingModelInfo(modelId) {
        this.ensureInitialized();
     ...
     * @inheritdoc
     */
    async getEmbeddingDimension(modelId) {
        this.ensureInitialized();
        ...
        throw new GMIError(`Embedding dimension for model '${modelId || this.defaultModel?.modelId || 'unknown'}' is not configured, and no system-wide default dimension is set. Please check EmbeddingManagerConfiguration.`, GMIErrorCode.CONFIG_ERROR, { modelIdAttempted: modelId || this.defaultModel?.modelId });
    }
    /**
     * @inheritdoc
     */
