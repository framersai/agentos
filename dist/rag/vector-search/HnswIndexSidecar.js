...
export class HnswIndexSidecar {
    ...
    async search(query, topK) {
        if (!this.index || this.labelToId.size === 0)
            return [];
     ...
     */
    async rebuildFromData(items) {
        if (!this.hnswAvailable || items.length === 0)
            return;
    ...
    async save() {
        ...
        try {
            ...
            await fs.writeFile(`${this.config.indexPath}.map.json`, JSON.stringify(mapData), 'utf8');
            this.dirty = false;
        }
        catch (err) {
    ...
    async load() {
        ...
        try {
            ...
            this.idToLabel = new Map(mapData.entries.map(([label, id]) => [id, label]));
            this.dirty = false;
            return true;
        }
        catch (err) {
...
//# sourceMappingURL=HnswIndexSidecar.js.map