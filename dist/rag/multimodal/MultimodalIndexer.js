 ...
 */
const DEFAULT_IMAGE_DESCRIPTION_PROMPT = 'Describe this image in detail for use in a search index. ' +
    'Include objects, actions, colors, text, spatial relationships, ' +
    'and any notable characteristics. Be thorough but concise.';
const isNodeBuffer = (value) => typeof Buffer !== 'undefined' && Buffer.isBuffer(value);
// ---------------------------------------------------------------------------
// Implementation
// ---------------------------------------------------------------------------
/**
...
export class MultimodalIndexer {
     ...
     * ```
     */
    async indexImage(opts) {
        if (!this._visionProvider) {
            throw new Error('MultimodalIndexer: cannot index image — no vision provider configured. ' +
     ...
     * ```
     */
    async indexAudio(opts) {
        if (!this._sttProvider) {
            throw new Error('MultimodalIndexer: cannot index audio — no STT provider configured. ' +
    ...
    async search(query, opts) {
        ...
        const filter = opts?.modalities && opts.modalities.length > 0
     ...
     */
    createMemoryBridge(memoryManager, options) {
        // Lazy import to avoid circular dependency at module load time.
...
//# sourceMappingURL=MultimodalIndexer.js.map