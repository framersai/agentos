...
For “lazy” skills, `SkillRegistry` from `@framers/agentos-skills` exposes tools that let the model fetch `SKILL.md` content on demand (via tool calls) instead of pre-injecting everything.
