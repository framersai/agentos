 ...
 */
export class FallbackAudioProxy implements IAudioGenerator {
   ...
   */
  constructor(
    private readonly chain: IAudioGenerator[],
    ...
    return this.runWithFallback(
      ...
      (provider) => {
        if (!provider.supports('music')) {
          throw Object.assign(
            new Error(`Provider "${provider.providerId}" does not support music generation.`),
            { name: 'MusicNotSupportedError' },
          );
        }
    ...
    return this.runWithFallback(
      ...
      (provider) => {
        if (!provider.supports('sfx') || typeof provider.generateSFX !== 'function') {
          throw Object.assign(
            new Error(`Provider "${provider.providerId}" does not support SFX generation.`),
            { name: 'SFXNotSupportedError' },
          );
        }
  ...
  private async runWithFallback(
    ...
    execute: (provider: IAudioGenerator) => Promise<AudioResult>,
  ): Promise<AudioResult> {
    ...
    for (let i = 0; i < this.chain.length; i++) {
      ...
      try {
        ...
        const isNotSupported =
          err instanceof Error && err.name.endsWith('NotSupportedError');
        ...
        if (i < this.chain.length - 1) {
          const event: AudioFallbackEvent = {
            type: 'audio:generate:fallback',
    ...
    throw new AggregateError(
      errors,
      `All ${this.chain.length} audio providers failed for ${operationName}: ${errors.map((e) => e.message).join('; ')}`,
    );
  }
}
