...
export class OpenRouterImageProvider implements IImageProvider {
  ...
  async generateImage(request: ImageGenerationRequest): Promise<ImageGenerationResult> {
    ...
    if (request.size) imageConfig.image_size = request.size;
    if (request.outputFormat) imageConfig.image_format = normalizeOutputFormat(request.outputFormat);
    if (providerOptions?.imageConfig) Object.assign(imageConfig, providerOptions.imageConfig);
    ...
    if (providerOptions) {
      ...
      if (transforms) body.transforms = transforms;
