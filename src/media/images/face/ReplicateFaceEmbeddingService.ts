 ...
 */
export class ReplicateFaceEmbeddingService implements IFaceEmbeddingService {
   ...
   */
  async extractEmbedding(imageUrl: string): Promise<FaceEmbedding> {
    const body = {
   ...
   */
  compareFaces(a: number[], b: number[], threshold = 0.6): FaceComparisonResult {
   ...
   */
  private async pollPrediction(url: string): Promise<ReplicatePrediction> {
   ...
   */
  private parseEmbeddingOutput(output: unknown): FaceEmbedding {
