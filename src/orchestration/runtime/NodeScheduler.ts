...
export class NodeScheduler {
  ...
  private predecessors: Map<string, string[]> = new Map();
  ...
  topologicalSort(): string[] {
    ...
    const realInDegree = new Map<string, number>();
    for (const id of this.nodeIds) {
      realInDegree.set(
        id,
        (this.predecessors.get(id) ?? []).filter(p => p !== START && p !== END).length,
      );
    }
    ...
    while (queue.length > 0) {
      ...
      for (const neighbour of this.adjacency.get(current) ?? []) {
        ...
        if (!this.nodeIds.has(neighbour)) continue;
