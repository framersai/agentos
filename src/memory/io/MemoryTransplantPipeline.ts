...
export interface TransplantOptions {
  mode: 'heuristic' | 'llm';
  sourceIdentity?: { name: string; pronouns?: string };
...
type TraceCategory = 'player_fact' | 'shared_experience' | 'self_referential' | 'world_knowledge';
...
function classifyTrace(trace: TraceRecord, sourceIdentity?: { name: string }): TraceCategory {
...
function rewriteSharedExperience(content: string): string {
  let result = content;
...
function addTransplantTags(trace: TraceRecord, sourceIdentity?: { name: string }): void {
  let tags: string[] = [];
  try {
...
export class MemoryTransplantPipeline {
  ...
  static async transform(
    ...
    try {
      payload = JSON.parse(brainJson) as Record<string, unknown>;
    } catch (err) {
    ...
    const traces = (payload.traces ?? []) as TraceRecord[];
    const outputTraces: TraceRecord[] = [];

    for (const trace of traces) {
      if (!trace.content || (trace as any).deleted === 1) {
