...
export type ReflectionPatternType =
  | 'preference'
  | 'behavior'
  | 'capability'
  | 'relationship'
...
export class ObservationReflector {
   ...
   */
  async reflect(observations: CompressedObservation[]): Promise<Reflection[]> {
    ...
    const userPrompt = observations
      .map((o) =>
        `[${o.id}] (priority=${o.priority}, importance=${o.importance.toFixed(2)}, ref=${o.temporal.relativeLabel}) ${o.summary}`,
      )
      .join('\n');
  ...
  private parseReflections(
    ...
    for (const item of parsed) {
      ...
      if (typeof obj.insight !== 'string' || !obj.insight) continue;

      const validPatternTypes: ReflectionPatternType[] = [
        'preference', 'behavior', 'capability', 'relationship', 'goal',
      ];
      const patternType = validPatternTypes.includes(obj.patternType as ReflectionPatternType)
        ? (obj.patternType as ReflectionPatternType)
        : 'behavior';
   ...
   */
  private buildSpanLabel(start: number, end: number, now: number): string {
    ...
    if (ageDays <= 1) return 'over the past day';
    if (ageDays <= 7) return `over the past ${ageDays} days`;
    if (ageDays <= 14) return 'over the past week';

    const ageWeeks = Math.floor(ageMs / WEEK);
    if (ageWeeks <= 4) return `over the past ${ageWeeks} weeks`;

    const ageMonths = Math.floor(ageDays / 30);
    if (ageMonths <= 1) return 'over the past month';
    if (ageMonths <= 12) return `over the past ${ageMonths} months`;
