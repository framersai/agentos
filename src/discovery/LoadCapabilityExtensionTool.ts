...
const INPUT_SCHEMA: JSONSchemaObject = {
  ...
  properties: {
    extensionId: {
      type: 'string',
      description:
        'Registry extension name to load at runtime (for example "omdb", "clearbit", or "web-scraper").',
    },
  },
 ...
 */
export function createLoadCapabilityExtensionTool(
  toolOrchestrator: IToolOrchestrator,
): ITool<LoadCapabilityExtensionInput, LoadCapabilityExtensionOutput> {
  return {
    id: 'agentos-load-capability-extension',
    name: 'load_capability_extension',
    displayName: 'Load Capability Extension',
    description:
      'Load a curated extension pack into the current runtime when you have identified an extension ID that is not yet active. ' +
    ...
    async execute(
      args: LoadCapabilityExtensionInput,
      _context: ToolExecutionContext,
    ): Promise<ToolExecutionResult<LoadCapabilityExtensionOutput>> {
      ...
      if (registeredTools.length === 0) {
        return {
          success: false,
          error:
            `Extension "${extensionId}" could not be loaded or did not register any tools.`,
        };
      }
