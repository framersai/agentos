...
export interface RTCDataChannelLike {
  ...
  // Event handlers — assigned as function properties (not addEventListener)
 ...
 * ```
 */
export class WebRTCStreamTransport extends EventEmitter implements IStreamTransport {
  // -------------------------------------------------------------------------
  ...
  private readonly _pc: RTCPeerConnectionLike;
  ...
  private readonly _config: WebRTCStreamTransportConfig;
   ...
   */
  private _attachAudioChannelHandlers(channel: RTCDataChannelLike): void {
   ...
   */
  private _attachPeerConnectionHandlers(): void {
    ...
    this._pc.onicecandidate = (event: { candidate: unknown }) => {
      if (event.candidate) {
    ...
    // and answerer roles.
    this._pc.ondatachannel = (event: { channel: RTCDataChannelLike }) => {
   ...
   */
  private _checkBothChannelsOpen(): void {
...
export async function createWebRTCTransport(
  ...
  return new WebRTCStreamTransport(pc as RTCPeerConnectionLike, config);
}
