...
export class ContentAdaptationEngine {
  ...
  private adaptForPlatform(
    ...
    if (hashtags && hashtags.length > constraints.maxHashtags) {
      warnings.push(
        `Hashtag count (${hashtags.length}) exceeds platform maximum (${constraints.maxHashtags}). ` +
          `Truncated to ${constraints.maxHashtags}.`,
      );
    }

    // Build the full text with hashtags placed according to platform style
    let fullText = this.formatWithHashtags(
      baseContent,
      normalizedHashtags,
      constraints.hashtagStyle,
    );
    ...
    if (fullText.length > constraints.maxLength) {
      ...
      if (
        constraints.hashtagStyle === 'footer' &&
        ...
        if (contentBudget > 0) {
          const truncatedContent = this.truncateWithEllipsis(
            baseContent,
            contentBudget,
          );
      ...
      warnings.push(
        `Content truncated from ${baseContent.length} to fit ${platform}'s ` +
    ...
    if (platform === 'bluesky' && hashtags && hashtags.length > 0) {
      warnings.push(
        'Bluesky uses facets (rich text) for hashtags, not inline text. ' +
    ...
    if (platform === 'mastodon') {
      warnings.push(
        'Consider adding a Content Warning (CW) if the content is sensitive.',
      );
    }
   ...
   */
  private buildHashtagBlock(hashtags: string[]): string {
   ...
   */
  private getDefaultConstraints(): PlatformConstraints {
    return {
      maxLength: 10000,
      hashtagStyle: 'inline',
      maxHashtags: 10,
