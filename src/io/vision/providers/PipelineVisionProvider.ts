 ...
 * @example
 * ```typescript
 * import { VisionPipeline, PipelineVisionProvider } from '@framers/agentos/io/vision';
...
import type { IVisionProvider } from '../../../cognition/rag/multimodal/types.js';
 ...
 * ```
 */
export class PipelineVisionProvider implements IVisionProvider {
  /**
   ...
   */
  private readonly _pipeline: VisionPipeline;
  ...
  constructor(pipeline: VisionPipeline) {
    if (!pipeline) {
      throw new Error(
        'PipelineVisionProvider: pipeline instance is required.',
      );
    }
   ...
   * ```
   */
  async processWithFullResult(image: Buffer | string): Promise<VisionResult> {
