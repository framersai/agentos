 ...
 * ```
 */
export class AudioGenLocalProvider implements IAudioGenerator {
  /** @inheritdoc */
  public readonly providerId = 'audiogen-local';
  ...
  private _modelId = 'Xenova/audiogen-medium';
  ...
  async generateMusic(_request: MusicGenerateRequest): Promise<AudioResult> {
    throw new Error(
      'AudioGen local provider does not support music generation. Use MusicGenLocalProvider instead.',
    );
  }
  ...
  private async _ensurePipeline(): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(
        'AudioGen local provider requires @huggingface/transformers. '
