 ...
 */
export function euclideanDistance(a: VectorLike, b: VectorLike): number {
  if (a.length !== b.length || a.length === 0) return 0;
 ...
 */
export function isLegacyJsonBlob(blob: Buffer | string): boolean {
  if (typeof blob === 'string') return true;
