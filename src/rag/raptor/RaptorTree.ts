...
export class RaptorTree {
  ...
  private minChunksForLayer: number;

  /** Cached statistics from the last build. */
  private stats: RaptorTreeStats;
  ...
  async build(
    ...
    let currentLayerChunks: RaptorInputChunk[] = [...chunks];
    let layerIndex = 1;

    while (
      layerIndex <= this.maxDepth &&
      ...
      if (!embeddingResponse.embeddings || embeddingResponse.embeddings.length === 0) {
        console.warn(
          `RaptorTree: Failed to embed layer ${layerIndex - 1} chunks. Stopping tree construction.`,
        );
        break;
      }
      ...
      const summaries: RaptorInputChunk[] = [];
      for (let ci = 0; ci < clusters.length; ci++) {
        const clusterIndices = clusters[ci];
        const clusterTexts = clusterIndices.map(
          (idx) => currentLayerChunks[idx].text,
        );
   ...
   * ```
   */
  async search(query: string, topK: number = 10): Promise<RaptorResult[]> {
    // Embed the query
    const embeddingResponse = await this.embeddingManager.generateEmbeddings({
      texts: [query],
    });
    ...
    const result: QueryResult = await this.vectorStore.query(
      this.collectionName,
      queryEmbedding,
      {
        topK: topK * 2,
    ...
    const raptorResults: RaptorResult[] = result.documents.map(
      (doc: RetrievedVectorDocument) => ({
        id: doc.id,
        text: doc.textContent ?? '',
        score: doc.similarityScore,
        layer: (doc.metadata?.raptorLayer as number) ?? 0,
