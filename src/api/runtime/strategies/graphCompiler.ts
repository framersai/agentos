...
function extractInstructions(
  ...
  if (!isAgent(agentOrConfig)) {
    return (agentOrConfig as BaseAgentConfig).instructions ?? `Agent "${name}"`;
  }
  return `Agent "${name}"`;
}
...
function makeAgentNode(
  ...
  agentName: string,
  instructions: string,
  scratchReadKeys: string[],
  scratchWriteKey: string,
): GraphNode {
...
function makeSynthesizerNode(
  ...
  return {
    ...
    executorConfig: {
      type: 'gmi',
      instructions:
        `You are a synthesis agent. Multiple agents have produced outputs stored in scratch keys: ${agentOutputKeys.join(', ')}. ` +
 ...
 */
function staticEdge(source: string, target: string): GraphEdge {
  return {
    id: nextEdgeId(),
    source,
    target,
 ...
 */
function compileSequentialGraph(
  agents: Record<string, BaseAgentConfig | Agent>,
  config: AgencyOptions,
  prompt: string,
): { nodes: GraphNode[]; edges: GraphEdge[]; reducers: StateReducers } {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const node: GraphNode = {
        ...
        executorConfig: {
          type: 'gmi',
          instructions:
            `You are agent "${name}" in round ${round + 1}/${maxRounds} of a debate. ` +
            `${instructions}\n\n` +
 ...
 * @returns The compiled graph components.
 */
function compileReviewLoopGraph(
  agents: Record<string, BaseAgentConfig | Agent>,
  config: AgencyOptions,
  prompt: string,
): { nodes: GraphNode[]; edges: GraphEdge[]; reducers: StateReducers } {
  ...
  const nodes: GraphNode[] = [
    // Producer: reads prompt + any reviewer feedback from scratch.
    {
      ...
      executorConfig: {
        type: 'gmi',
        instructions:
          `You are the producer agent "${producerName}". ${producerInstructions}\n\n` +
    ...
    // Reviewer: reads the draft and decides approval.
    {
      ...
      executorConfig: {
        type: 'gmi',
        instructions:
          `You are the reviewer agent "${reviewerName}". ${reviewerInstructions}\n\n` +
    ...
    // Routes to END if approved or rounds exhausted, back to producer otherwise.
    {
      ...
      executorConfig: {
        ...
        condition: {
          type: 'function',
          fn: (state) => {
            const scratch = state.scratch as Record<string, unknown>;
            const approved = scratch.reviewApproved === true;
            const round = (scratch.reviewRound as number) ?? 0;
 ...
 * @returns The compiled graph components.
 */
function compileHierarchicalGraph(
  agents: Record<string, BaseAgentConfig | Agent>,
  config: AgencyOptions,
  prompt: string,
): { nodes: GraphNode[]; edges: GraphEdge[]; reducers: StateReducers } {
 ...
 * ```
 */
export function compileAgencyToGraph(
  config: AgencyOptions,
  prompt: string,
): CompiledExecutionGraph {
  resetEdgeCounter();
  ...
  let compiled: { nodes: GraphNode[]; edges: GraphEdge[]; reducers: StateReducers };
 ...
 */
export function mapGraphResultToAgencyResult(
  finalOutput: unknown,
  config: AgencyOptions,
): Record<string, unknown> {
  const output = finalOutput as Record<string, unknown> | null;

  // Extract text from various possible locations in the output.
  const text =
    (output?.finalOutput as string) ??
 ...
 */
export function mapGraphEventToAgencyEvent(
