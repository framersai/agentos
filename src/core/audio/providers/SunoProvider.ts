...
/**
 * Configuration for the Suno music generation provider.
 *
 * @example
 * ```typescript
 * const config: SunoProviderConfig = {
 *   apiKey: process.env.SUNO_API_KEY ?? process.env.REPLICATE_API_TOKEN!,
 * };
 * ```
 */
export interface SunoProviderConfig {
  /**
   * Replicate API token. Sent as `Authorization: Token ${apiKey}`.
   * Accepts either SUNO_API_KEY or REPLICATE_API_TOKEN.
   */
  apiKey: string;
 ...
 * ```
 */
export class SunoProvider implements IAudioGenerator {
  /** @inheritdoc */
  public readonly providerId = 'suno';
  ...
  async generateSFX(_request: SFXGenerateRequest): Promise<AudioResult> {
    throw new Error(
      'Suno provider does not support SFX generation. Use an SFX-capable provider.',
    );
  }
  ...
  private async _pollPrediction(url: string): Promise<ReplicatePrediction> {
    ...
    throw new Error(
      `Suno music generation timed out after ${this._config.timeoutMs}ms.`,
    );
  }
