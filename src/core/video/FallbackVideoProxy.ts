 ...
 * transiently failing.
 */
export class FallbackVideoProxy implements IVideoGenerator {
   ...
   */
  constructor(
    private readonly chain: IVideoGenerator[],
  ...
  async generateVideo(request: VideoGenerateRequest): Promise<VideoResult> {
    return this.runWithFallback(
      'generateVideo',
      (provider) => provider.generateVideo(request),
    );
  }
    ...
    return this.runWithFallback(
      ...
      (provider) => {
        if (!provider.supports('image-to-video') || typeof provider.imageToVideo !== 'function') {
          throw Object.assign(
            new Error(`Provider "${provider.providerId}" does not support imageToVideo.`),
            { name: 'ImageToVideoNotSupportedError' },
          );
        }
  ...
  private async runWithFallback(
    ...
    execute: (provider: IVideoGenerator) => Promise<VideoResult>,
  ): Promise<VideoResult> {
    ...
    for (let i = 0; i < this.chain.length; i++) {
      ...
      try {
        ...
        if (i < this.chain.length - 1) {
          const event: VideoFallbackEvent = {
            type: 'video:generate:fallback',
    ...
    throw new AggregateError(
      errors,
      `All ${this.chain.length} video providers failed for ${operationName}: ${errors.map((e) => e.message).join('; ')}`,
    );
  }
}
